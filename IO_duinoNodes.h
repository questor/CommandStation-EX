...
public:
  IO_duinoNodes(VPIN firstVpin, int nPins, byte clockPin, byte latchPin,
                byte dataPin, const byte *pinmap)
  ...
  void _display() override {
    DIAG(F("IO_duinoNodes %SPUT Configured on Vpins:%u-%u shift=%d"),
         _pinMap ? F("IN") : F("OUT"), (int)_firstVpin,
...
class IO_DNIN8 {
public:
  static void create(VPIN firstVpin, int nPins, byte clockPin, byte latchPin,
                     byte dataPin) {
    // input arrives as board pin 0,7,6,5,1,2,3,4
    static const byte pinmap[8] = {0x80, 0x01, 0x02, 0x04,
                                   0x40, 0x20, 0x10, 0x08};
...
public:
  static void create(VPIN firstVpin, int nPins, byte clockPin, byte latchPin,
    ...
    static const byte pinmap[8] = {0x01, 0x02, 0x04, 0x08,
                                   0x10, 0x20, 0x40, 0x80};
    if (IODevice::checkNoOverlap(firstVpin, nPins))
      new IO_duinoNodes(firstVpin, nPins, clockPin, latchPin, dataPin, pinmap);
  }
