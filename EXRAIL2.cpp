...
/* EXRAILPlus planned TRANSPARENT additions
   T1. [DONE] RAM based fast lookup for sequences ON* event catchers and
   signals. T2. Extend to >64k
  */
...
// Each thread manages a loco on a journey through the layout, and/or may manage
// a scenery automation. The threads exist in a ring, each time through loop()
// the next thread in the ring is serviced.

// Statics
const int16_t LOCO_ID_WAITING = -99; // waiting for loco id from prog track
int16_t RMFT2::progtrackLocoId; // used for callback when detecting a loco on
                                // prog track
bool RMFT2::diag = false;       // <D EXRAIL ON>
RMFT2 *RMFT2::loopTask =
    NULL; // loopTask contains the address of ONE of the tasks in a ring.
RMFT2 *RMFT2::pausingTask =
    NULL; // Task causing a PAUSE.
          // when pausingTask is set, that is the ONLY task that gets any
          // service, and all others will have their locos stopped, then resumed
          // after the pausing task resumes.
...
uint16_t RMFT2::getOperand(byte n) { return getOperand(progCounter, n); }
...
/* static */ void RMFT2::begin() {
  ...
  // create lookups
  sequenceLookup =
      LookListLoader(OPCODE_ROUTE, OPCODE_AUTOMATION, OPCODE_SEQUENCE);
...
void RMFT2::ComandFilter(Print *stream, byte &opcode, byte &paramCount,
                         int16_t p[]) {
...
bool RMFT2::parseSlash(Print *stream, byte &paramCount, int16_t p[]) {
  ...
  if (paramCount == 0) { // STATUS
    ...
    for (int id = 0; id < MAX_FLAGS; id++) {
      byte flag = flags[id];
      if (flag & ~TASK_FLAG & ~SIGNAL_MASK) { // not interested in TASK_FLAG
                                              // only. Already shown above
        StringFormatter::send(stream, F("\nflags[%d] "), id);
    ...
    for (int sigslot = 0;; sigslot++) {
      ...
      if (sigid == 0)
        break; // end of signal list
      byte flag =
          flags[sigslot] & SIGNAL_MASK; // obtain signal flags for this id
      StringFormatter::send(stream, F("\n%S[%d]"),
                            (flag == SIGNAL_RED)     ? F("RED")
                            : (flag == SIGNAL_GREEN) ? F("GREEN")
                                                     : F("AMBER"),
  ...
  if (p[0] == HASH_KEYWORD_KILL && p[1] == HASH_KEYWORD_ALL) {
    while (loopTask)
      loopTask->kill(F("KILL ALL")); // destructor changes loopTask
    return true;
  }
...
void RMFT2::driveLoco(byte speed) {
  ...
  if (diag)
    DIAG(F("EXRAIL drive %d %d %d"), loco, speed, forward ^ invert);
  /* TODO.....
...
bool RMFT2::readSensor(uint16_t sensorId) {
  // Exrail operands are unsigned but we need the signed version as inserted by
  // the macros.
  ...
  // negative sensorIds invert the logic (e.g. for a break-beam sensor which
  // goes OFF when detecting)
...
/* static */ void RMFT2::readLocoCallback(int16_t cv) {
  if (cv & LONG_ADDR_MARKER) { // maker bit indicates long addr
    progtrackLocoId =
        cv ^ LONG_ADDR_MARKER; // remove marker bit to get real long addr
...
void RMFT2::loop2() {
  ...
  //            Breaking from this switch will step to the next step in the
  //            route.
  ...
  case OPCODE_AFTER: // waits for sensor to hit and then remain off for 0.5
                     // seconds. (must come after an AT operation)
    if (readSensor(operand)) {
  ...
  case OPCODE_SET_TRACK:
    ...
    // If DC/DCX use  my loco for DC address
    {
      ...
      int16_t cab =
          (mode == TRACK_MODE_DC || mode == TRACK_MODE_DCX) ? loco : 0;
  ...
  case OPCODE_IFLOCO:                   // do if the loco is the active one
    skipIf = loco != (uint16_t)operand; // bad luck if someone enters negative
                                        // loco numbers into EXRAIL
    break;
  ...
  case OPCODE_SERVO: // OPCODE_SERVO,V(vpin),OPCODE_PAD,V(position),OPCODE_PAD,V(profile),OPCODE_PAD,V(duration)
    IODevice::writeAnalogue(operand, getOperand(1), getOperand(2),
                            getOperand(3));
    break;
  ...
  case OPCODE_PAD:     // Just a padding for previous opcode needing >1 operand
                       // byte.
...
void RMFT2::kill(const FSH *reason, int operand) {
  if (reason)
    DIAG(F("EXRAIL ERROR pc=%d, cab=%d, %S %d"), progCounter, loco, reason,
         operand);
  else if (diag)
...
/* static */ void RMFT2::doSignal(int16_t id, char rag) {
  if (diag)
    DIAG(F(" doSignal %d %x"), id, rag);

  // Schedule any event handler for this signal change.
  // Thjis will work even without a signal definition.
  if (rag == SIGNAL_RED)
    handleEvent(F("RED"), onRedLookup, id);
  else if (rag == SIGNAL_GREEN)
    handleEvent(F("GREEN"), onGreenLookup, id);
  else
    handleEvent(F("AMBER"), onAmberLookup, id);
  ...
  if (diag)
    DIAG(F("signal %d %d %d %d %d"), sigid, id, redpin, amberpin, greenpin);
  ...
  if (sigtype == SERVO_SIGNAL_FLAG) {
    ...
    int16_t servopos = rag == SIGNAL_RED
                           ? redpin
                           : (rag == SIGNAL_GREEN ? greenpin : amberpin);
    if (diag)
      DIAG(F("sigA %d %d"), id, servopos);
    if (servopos != 0)
  ...
  if (rag == SIGNAL_AMBER && (amberpin == 0))
...
void RMFT2::turnoutEvent(int16_t turnoutId, bool closed) {
  ...
  if (closed)
    handleEvent(F("CLOSE"), onCloseLookup, turnoutId);
  else
    handleEvent(F("THROW"), onThrowLookup, turnoutId);
}

void RMFT2::activateEvent(int16_t addr, bool activate) {
  // Hunt for an ONACTIVATE/ONDEACTIVATE for this accessory
  if (activate)
    handleEvent(F("ACTIVATE"), onActivateLookup, addr);
  else
    handleEvent(F("DEACTIVATE"), onDeactivateLookup, addr);
}

void RMFT2::changeEvent(int16_t vpin, bool change) {
  // Hunt for an ONCHANGE for this sensor
  if (change)
    handleEvent(F("CHANGE"), onChangeLookup, vpin);
}
...
void RMFT2::thrungeString(uint32_t strfar, thrunger mode, byte id) {
  ...
  case thrunge_parse:
    ...
    break;
  case thrunge_broadcast:
    CommandDistributor::broadcastRaw(CommandDistributor::COMMAND_TYPE,
                                     buffer->getString());
    break;
  case thrunge_withrottle:
    CommandDistributor::broadcastRaw(CommandDistributor::WITHROTTLE_TYPE,
                                     buffer->getString());
    break;
  case thrunge_lcd:
  ...
  default: // thrunge_lcd+1, ...
    if (mode > thrunge_lcd)
      SCREEN(mode - thrunge_lcd, id, F("%s"),
             buffer->getString()); // print to other display
    break;
  }
}
