 ...
 *  I2CManager.loop();
 * So that the application doesn't need to do this explicitly, this call is
 * performed from the I2CRB::isBusy() or I2CRB::wait() functions.
 *
 */
 ...
 *
 *  A multiplexer offers a way of extending the address range of I2C devices.
 * For example, GPIO extenders use address range 0x20-0x27 to are limited to 8
 * on a bus.  By adding a multiplexer, the limit becomes 8 for each of the
 * multiplexer's 8 sub-buses, i.e. 64. And a single I2C bus can have up to 8
 * multiplexers, giving up to 64 sub-buses and, in theory, up to 512 I/O
 * extenders; that's as many as 8192 input/output pins! Secondly, the
 * capacitance of the bus is an electrical limiting factor of the length of the
 * bus, speed and number of devices. The multiplexer isolates each sub-bus from
 * the others, and so reduces the capacitance of the bus.  For example, with one
 *  multiplexer and 64 GPIO extenders, only 9 devices are connected to the bus
 * at any time (multiplexer plus 8 extenders). Thirdly, the multiplexer offers
 * the ability to use mixed-speed devices more effectively, by allowing
 * high-speed devices to be put on a different bus to low-speed devices,
 * enabling the software to switch the I2C speed on-the-fly between I2C
 * transactions.
 *
 *
 *  Non-interrupting I2C:
 *
 *  Non-blocking I2C may be operated without interrupts (undefine
 * I2C_USE_INTERRUPTS).  Instead, the I2C state machine handler, currently
 * invoked from the interrupt service routine, is invoked from the loop()
 * function. The speed at which I2C operations can be performed then becomes
 * highly dependent on the frequency that the loop() function is called, and may
 * be adequate under some circumstances. The advantage of NOT using interrupts
 * is that the impact of I2C upon the DCC waveform (when accurate timing mode
 * isn't in use) becomes almost zero.
 *
 */
...
// Add following line to config.h to enable Wire library instead of native I2C
// drivers
// #define I2C_USE_WIRE

// Add following line to config.h to disable the use of interrupts by the native
// I2C drivers.
...
// I2C Extended Address support I2C Multiplexers and allows various properties
// to be associated with an I2C address such as the MUX and SubBus.  In the
// future, this may be extended to include multiple buses, and other features.
...
// Currently only one bus supported, and one instance of I2CManager to handle
// it.
enum I2CBus : uint8_t {
  I2CBus_0 = 0,
};
...
public:
  ...
  I2CAddress(const I2CBus busNumber, const I2CMux muxNumber,
             const I2CSubBus subBus, const uint8_t deviceAddress) {
  ...
  I2CAddress(I2CAddress firstAddress, uint8_t newDeviceAddress)
      : I2CAddress(firstAddress._muxNumber, firstAddress._subBus,
                   newDeviceAddress) {}

  // Conversion operator from I2CAddress to uint8_t
  // For "uint8_t address = i2cAddress;" syntax
  // (device assumed to be on the main I2C bus or on a currently selected
  // subbus.
...
private:
  // Helper function for converting byte to four-character hex string (e.g.
  // 0x23).
...
public:
  // Constructors
  I2CAddress(const uint8_t deviceAddress) { _deviceAddress = deviceAddress; }
...
private:
  ...
  void addressWarning() {
    if (!_addressWarningDone) {
      DIAG(F("WARNIING: Extended I2C address used but not supported in this "
             "configuration"));
...
public:
  volatile uint8_t
      status; // Completion status, or pending flag (updated from IRC)
  ...
  void setReadParams(I2CAddress i2cAddress, uint8_t *readBuffer,
                     uint8_t readLen);
  void setRequestParams(I2CAddress i2cAddress, uint8_t *readBuffer,
                        uint8_t readLen, const uint8_t *writeBuffer,
                        uint8_t writeLen);
  void setWriteParams(I2CAddress i2cAddress, const uint8_t *writeBuffer,
                      uint8_t writeLen);
  ...
  I2CRB
      *nextRequest; // Used by non-blocking devices for I2CRB queue management.
#endif
};
...
public:
  ...
  // Select/deselect Mux Sub-Bus (if using legacy addresses, just checks
  // address) E.g. muxSelectSubBus({I2CMux_0, SubBus_3});
  uint8_t muxSelectSubBus(I2CAddress address) { return checkAddress(address); }
...
private:
  ...
  // Clock speed must be no higher than 400kHz on AVR. Higher is possible on
  // 4809, SAMD and STM32 but most popular I2C devices are 400kHz so in practice
  // the higher speeds will not be useful.  The speed can be overridden by
  // I2CManager::forceClock().
...
public:
  ...
  // If there is only one request, then queueHead and queueTail both point to
  // it. Otherwise, queueHead is the pointer to the first request in the queue
  // and queueTail is the pointer to the last request in the queue. Within the
  // queue, each request's nextRequest field points to the next request, or
  // NULL. Mark volatile as they are updated by IRC and read/written elsewhere.
private:
...
// Pointer to class instance (Note: if there is more than one bus, each will
// have its own instance of I2CManager, selected by the queueRequest function
// from the I2CBus field within the request block's I2CAddress).
