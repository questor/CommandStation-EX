...
// This file will include and build the EXRAIL script and associated helper
// tricks. It does this by including myAutomation.h several times, each with a
// set of macros to extract the relevant parts.

// The entire automation script is contained within a byte array
// RMFT2::RouteCode[] made up of opcode and parameter pairs. ech opcode is a 1
// byte operation plus 2 byte operand. The array is normally built using the
// macros below as this makes it easier to manage the cases where:
// - padding must be applied to ensure the correct alignment of the next
// instruction
// - large parameters must be split up
// - multiple parameters aligned correctly
// - a single macro requires multiple operations

// Descriptive texts for routes and animations are created in a sepaerate
// function which can be called to emit a list of routes/automatuions in a form
// suitable for Withrottle.

// PRINT(msg), LCD(row,msg) and SCREEN(display,row,msg) are implemented in a
// separate pass to create a getMessageText(id) function.
...
#define ALIAS(name, value...)                                                  \
  const int name = 1##value##0 == 10 ? -__COUNTER__ : value##0 / 10;
...
#undef SERVO_SIGNAL
#define SERVO_SIGNAL(vpin, redval, amberval, greenval)                         \
  vpin | RMFT2::SERVO_SIGNAL_FLAG, redval, amberval, greenval,
#undef DCC_SIGNAL
#define DCC_SIGNAL(id, addr, subaddr)                                          \
  id | RMFT2::DCC_SIGNAL_FLAG, addr, subaddr, 0,
...
// a constant.
#define V(val)                                                                 \
  (byte)(((int16_t)(val)) & 0x00FF), (byte)(((int16_t)(val) >> 8) & 0x00FF)
// Define macros for route code creation

#define ACTIVATE(addr, subaddr)                                                \
  OPCODE_DCCACTIVATE, V(addr << 3 | subaddr << 1 | 1),
#define ACTIVATEL(addr) OPCODE_DCCACTIVATE, V((addr + 3) << 1 | 1),
#define AFTER(sensor_id) OPCODE_AT, V(sensor_id), OPCODE_AFTER, V(sensor_id),
#define ALIAS(name, value...)
#define AMBER(signal_id) OPCODE_AMBER, V(signal_id),
#define ANOUT(vpin, value, param1, param2)                                     \
  OPCODE_SERVO, V(vpin), OPCODE_PAD, V(value), OPCODE_PAD, V(param1),          \
      OPCODE_PAD, V(param2),
#define AT(sensor_id) OPCODE_AT, V(sensor_id),
#define ATGTE(sensor_id, value)                                                \
  OPCODE_ATGTE, V(sensor_id), OPCODE_PAD, V(value),
#define ATLT(sensor_id, value) OPCODE_ATLT, V(sensor_id), OPCODE_PAD, V(value),
#define ATTIMEOUT(sensor_id, timeout)                                          \
  OPCODE_ATTIMEOUT1, 0, 0, OPCODE_ATTIMEOUT2, V(sensor_id), OPCODE_PAD,        \
      V(timeout / 100L),
...
#define DEACTIVATE(addr, subaddr)                                              \
  OPCODE_DCCACTIVATE, V(addr << 3 | subaddr << 1),
#define DEACTIVATEL(addr) OPCODE_DCCACTIVATE, V((addr + 3) << 1),
#define DELAY(ms)                                                              \
  ms < 30000 ? OPCODE_DELAYMS : OPCODE_DELAY, V(ms / (ms < 30000 ? 1L : 100L)),
#define DELAYMINS(mindelay) OPCODE_DELAYMINS, V(mindelay),
#define DELAYRANDOM(mindelay, maxdelay)                                        \
  DELAY(mindelay) OPCODE_RANDWAIT, V((maxdelay - mindelay) / 100L),
...
#define FADE(pin, value, ms)                                                   \
  OPCODE_SERVO, V(pin), OPCODE_PAD, V(value), OPCODE_PAD,                      \
      V(PCA9685::ProfileType::UseDuration | PCA9685::NoPowerOff), OPCODE_PAD,  \
      V(ms / 100L),
...
#define IFGTE(sensor_id, value)                                                \
  OPCODE_IFGTE, V(sensor_id), OPCODE_PAD, V(value),
...
#define MOVETT(id, steps, activity)                                            \
  OPCODE_SERVO, V(id), OPCODE_PAD, V(steps), OPCODE_PAD,                       \
      V(EXTurntable::activity), OPCODE_PAD, V(0),
...
#define ONCLOCKTIME(hours, mins)                                               \
  OPCODE_ONTIME, V((STRIP_ZERO(hours) * 60) + STRIP_ZERO(mins)),
...
#define PIN_TURNOUT(id, pin, description...)                                   \
  OPCODE_PINTURNOUT, V(id), OPCODE_PAD, V(pin),
#ifndef DISABLE_PROG
...
#define SERVO(id, position, profile)                                           \
  OPCODE_SERVO, V(id), OPCODE_PAD, V(position), OPCODE_PAD,                    \
      V(PCA9685::profile), OPCODE_PAD, V(0),
#define SERVO2(id, position, ms)                                               \
  OPCODE_SERVO, V(id), OPCODE_PAD, V(position), OPCODE_PAD,                    \
      V(PCA9685::Instant), OPCODE_PAD, V(ms / 100L),
#define SERVO_SIGNAL(vpin, redpos, amberpos, greenpos)
#define SERVO_TURNOUT(id, pin, activeAngle, inactiveAngle, profile,            \
                      description...)                                          \
  OPCODE_SERVOTURNOUT, V(id), OPCODE_PAD, V(pin), OPCODE_PAD, V(activeAngle),  \
      OPCODE_PAD, V(inactiveAngle), OPCODE_PAD,                                \
      V(PCA9685::ProfileType::profile),
#define SET(pin) OPCODE_SET, V(pin),
#define SET_TRACK(track, mode)                                                 \
  OPCODE_SET_TRACK, V(TRACK_MODE_##mode << 8 | TRACK_NUMBER_##track),
...
#define TURNOUT(id, addr, subaddr, description...)                             \
  OPCODE_TURNOUT, V(id), OPCODE_PAD, V(addr), OPCODE_PAD, V(subaddr),
#define TURNOUTL(id, addr, description...)                                     \
  TURNOUT(id, (addr - 1) / 4 + 1, (addr - 1) % 4, description)
#define UNJOIN OPCODE_UNJOIN, 0, 0,
#define UNLATCH(sensor_id) OPCODE_UNLATCH, V(sensor_id),
#define VIRTUAL_SIGNAL(id)
#define VIRTUAL_TURNOUT(id, description...)                                    \
  OPCODE_PINTURNOUT, V(id), OPCODE_PAD, V(0),
