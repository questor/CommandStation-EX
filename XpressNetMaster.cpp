...
bool XpressNetMasterClass::update(void) 
{
	...
	if (XNetRXBuffer.get != XNetRXBuffer.put) {
			...
			bool crcOk = XNetCheckXOR();
			char tmp[128];
			int wPos = 0;
			char *c = "0123456789ABCDEF";
			for (byte i = 0; i < ((XNetRXBuffer.msg[XNetRXBuffer.get].data[XNetheader] & 0x0F) + 3); i++) {
				byte d = XNetRXBuffer.msg[XNetRXBuffer.get].data[i];
				tmp[wPos++] = c[(d&0xf0)>>4];
				tmp[wPos++] = c[d&0xf];
				tmp[wPos++] = ' ';
			}
			tmp[wPos] = 0;
			DIAG(F("READ %s: %s"), &tmp[0], crcOk?"OK":"NOT OK");

			if (crcOk)	{ //Checks the XOR
				#if defined (XNetDEBUGTime)
