...
void I2C1_IRQHandler() { I2CManager.handleInterrupt(); }
#endif

// Assume I2C1 for now - default I2C bus on Nucleo-F411RE and likely Nucleo-64
// variants
I2C_TypeDef *s = I2C1;
...
// #define I2C_SR1_BERR      (1<<8)    // Bus error (misplaced start or stop
// condition) #define I2C_SR1_TxE       (1<<7)    // Data register empty on
// transmit #define I2C_SR1_RxNE      (1<<6)    // Data register not empty on
// receive #define I2C_SR1_STOPF     (1<<4)    // Stop detection (slave mode)
// #define I2C_SR1_ADD10     (1<<3)    // 10 bit header sent
// #define I2C_SR1_BTF       (1<<2)    // Byte transfer finished - data transfer
// done #define I2C_SR1_ADDR      (1<<1)    // Address sent (master) or matched
// (slave) #define I2C_SR1_SB        (1<<0)    // Start bit (master mode)
// 1=start condition generated

// I2C CR1 Control Register #1 bit definitions for convenience
// #define I2C_CR1_SWRST     (1<<15)   // Software reset - places peripheral
// under reset #define I2C_CR1_ALERT     (1<<13)   // SMBus alert assertion
// #define I2C_CR1_PEC       (1<<12)   // Packet Error Checking transfer in
// progress #define I2C_CR1_POS       (1<<11)   // Acknowledge/PEC Postion (for
// data reception in PEC mode) #define I2C_CR1_ACK       (1<<10)   //
// Acknowledge enable - ACK returned after byte is received (address or data)
...
// #define I2C_CR1_ENGC      (1<<6)    // General call (broadcast) enable
// (address 00h is ACKed) #define I2C_CR1_ENPEC     (1<<5)    // PEC Enable
...
void I2CManagerClass::I2C_setClock(uint32_t i2cClockSpeed) {
  ...
  // Bit 11-0: FREQR = 16MHz => TPCLK1 = 62.5ns, so CCR divisor must be 0x50 (80
  // * 62.5ns = 5000ns)
...
void I2CManagerClass::I2C_init() {
  ...
  GPIOB->MODER |=
      (2 << (8 * 2)) | (2 << (9 * 2)); // PB8 and PB9 set to ALT function
  GPIOB->OTYPER |=
      (1 << 8) | (1 << 9); // PB8 and PB9 set to open drain output capability
  GPIOB->OSPEEDR |=
      (3 << (8 * 2)) | (3 << (9 * 2)); // PB8 and PB9 set to High Speed mode
  GPIOB->PUPDR |=
      (1 << (8 * 2)) | (1 << (9 * 2)); // PB8 and PB9 set to pull-up capability
  ...
  GPIOB->AFR[1] |=
      (4 << 0) | (4 << 4); // PB8 on low nibble, PB9 on next nibble up

  // Software reset the I2C peripheral
  s->CR1 |= I2C_CR1_SWRST;    // reset the I2C
  s->CR1 &= ~(I2C_CR1_SWRST); // Normal operation

  // Program the peripheral input clock in CR2 Register in order to generate
  // correct timings
...
void I2CManagerClass::I2C_sendStart() {
  ...
  if (operation == OPERATION_READ ||
    ...
    if (bytesToReceive == 1) {
      ...
      temp =
          I2C1->SR1 |
          I2C1->SR2; // read SR1 and SR2 to clear the ADDR bit.... EV6 condition
 ...
 *  if I2C_USE_INTERRUPTS isn't defined, from the I2CManagerClass::loop()
 *function (and therefore, indirectly, from I2CRB::wait() and I2CRB::isBusy()).
 ***************************************************************************/
void I2CManagerClass::I2C_handleInterrupt() {
  ...
  if (s->SR1 && I2C_SR1_ARLO) {
    ...
    if (s->SR1 && (1 << 10)) {
      ...
      // and read bit. s->I2CM.ADDR.bit.ADDR = (deviceAddress << 1) | 1;
    } else {
      // Check both TxE/BTF == 1 before generating stop
      while (!(s->SR1 && I2C_SR1_TXE))
        ; // Check TxE
