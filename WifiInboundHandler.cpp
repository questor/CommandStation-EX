...
void WifiInboundHandler::loop() { singleton->loop1(); }
...
void WifiInboundHandler::loop1() {
  ...
  if (pendingCipsend && millis() - lastCIPSEND > CIPSENDgap) {
    if (Diag::WIFI)
      DIAG(F("WiFi: [[CIPSEND=%d,%d]]"), clientPendingCIPSEND,
           currentReplySize);
...
// This is a Finite State Automation (FSA) handling the inbound bytes from an ES
...
WifiInboundHandler::INBOUND_STATE WifiInboundHandler::loop2() {
  while (wifiStream->available()) {
    ...
    case ANYTHING: // looking for +IPD, > , busy ,  n,CONNECTED, n,CLOSED,
      ...
      if (ch ==
          'b') { // This is a busy indicator... probabaly must restart a CIPSEND
...
void WifiInboundHandler::purgeCurrentCIPSEND() {
  ...
  DIAG(F("Wifi: DROPPING CIPSEND=%d,%d"), clientPendingCIPSEND,
       currentReplySize);
  for (int i = 0; i < currentReplySize; i++)
    outboundRing->read();
  pendingCipsend = false;
