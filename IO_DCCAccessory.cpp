...
void DCCAccessoryDecoder::create(VPIN firstVpin, int nPins, int DCCAddress,
                                 int DCCSubaddress) {
...
// Constructors
DCCAccessoryDecoder::DCCAccessoryDecoder(VPIN vpin, int nPins, int DCCAddress,
                                         int DCCSubaddress) {
  _firstVpin = vpin;
  _nPins = nPins;
...
// Device-specific write function.  State 1=closed, 0=thrown.  Adjust for
// RCN-213 compliance
void DCCAccessoryDecoder::_write(VPIN id, int state) {
  ...
  state = !state;
#ifdef DIAG_IO
  DIAG(F("DCC Write Linear Address:%d State:%d (inverted)"), packedAddress,
       state);
#endif
#else
#ifdef DIAG_IO
...
void DCCAccessoryDecoder::_display() {
  ...
  DIAG(
      F("DCCAccessoryDecoder Configured on Vpins:%u-%u Addresses %d/%d-%d/%d)"),
      _firstVpin, _firstVpin + _nPins - 1, ADDRESS(_packedAddress),
