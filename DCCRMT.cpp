...
void setEOT(rmt_item32_t *item) { item->val = 0; }
...
RMTChannel::RMTChannel(pinpair pins, bool isMain) {
  ...
  config.mem_block_num =
      2; // With longest DCC packet 11 inc checksum (future expansion)
  ...
  */
  ...
  DIAG(F("Channel %d DCC signal for %s start"), config.channel,
       isMain ? "MAIN" : "PROG");
...
int RMTChannel::RMTfillData(const byte buffer[], byte byteCount,
                            byte repeatCount = 0) {
  ...
  if (dataReady == true) // the packet is not copied out yet
    return 1000;
  if (DATA_LEN(byteCount) >
      maxDataLen) { // this would overun our allocated memory for data
    DIAG(F("Can not convert DCC bytes # %d to DCC bits %d, buffer too small"),
         byteCount, maxDataLen);
