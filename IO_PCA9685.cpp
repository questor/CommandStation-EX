...
static const byte PCA9685_FIRST_SERVO =
    0x06; /** low byte first servo register ON*/
static const byte PCA9685_PRESCALE =
    0xFE; /** Prescale register for PWM output frequency */
// MODE1 bits
...
static const float FREQUENCY_OSCILLATOR =
    25000000.0; /** Accurate enough for our purposes  */
static const uint32_t MAX_I2C_SPEED =
    1000000L; // PCA9685 rated up to 1MHz I2C clock speed
...
bool PCA9685::_configure(VPIN vpin, ConfigTypeEnum configType, int paramCount,
                         int params[]) {
  ...
  if (paramCount != 5)
    return false;
#ifdef DIAG_IO
  DIAG(F("PCA9685 Configure VPIN:%u Apos:%d Ipos:%d Profile:%d Duration:%d "
         "state:%d"),
...
PCA9685::PCA9685(VPIN firstVpin, int nPins, I2CAddress i2cAddress,
  ...
  // To save RAM, space for servo configuration is not allocated unless a pin is
  // used. Initialise the pointers to NULL.
...
void PCA9685::_begin() {
  I2CManager.begin();
  I2CManager.setClock(
      1000000); // Nominally able to run up to 1MHz on I2C
  ...
  if (I2CManager.exists(_I2CAddress)) {
    ...
    // In theory, we should wait 500us before sending any other commands to each
    // device, to allow the PWM oscillator to get running.  However, we don't do
    // any specific wait, as there's plenty of other stuff to do before we will
    // send a command.
#if defined(DIAG_IO)
    _display();
#endif
...
void PCA9685::_write(VPIN vpin, int value) {
  ...
  if (s != NULL) {
    ...
    _writeAnalogue(vpin, value ? s->activePosition : s->inactivePosition,
...
//
void PCA9685::_writeAnalogue(VPIN vpin, int value, uint8_t profile,
                             uint16_t duration) {
#ifdef DIAG_IO
...
void PCA9685::writeDevice(uint8_t pin, int value) {
#ifdef DIAG_IO
  DIAG(F("PCA9685 I2C:%s WriteDevice Pin:%d Value:%d"), _I2CAddress.toString(),
       pin, value);
#endif
  ...
  if (status != I2C_STATUS_OK) {
    _deviceState = DEVSTATE_FAILED;
    DIAG(F("PCA9685 I2C:%s failed %S"), _I2CAddress.toString(),
...
void PCA9685::_display() {
  DIAG(F("PCA9685 I2C:%s Configured on Vpins:%u-%u %S"), _I2CAddress.toString(),
...
// The profile below is in the range 0-100% and should be combined with the
// desired limits of the servo set by _activePosition and _inactivePosition. The
// profile is symmetrical here, i.e. the bounce is the same on the down action
// as on the up action.  First entry isn't used.
const uint8_t FLASH PCA9685::_bounceProfile[30] = {
    0,  2,  3,  7,   13, 33, 50, 83, 100, 83, 75, 70, 65, 60, 60,
