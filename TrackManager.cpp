...
byte TrackManager::tempProgTrack =
    MAX_TRACKS + 1; // MAX_TRACKS+1 is the unused flag
#endif
...
void TrackManager::sampleCurrent() {
  ...
  if (!waiting) {
    ...
    while (true) {
      if (track[tr]->getMode() &
          (TRACK_MODE_MAIN | TRACK_MODE_PROG | TRACK_MODE_DC | TRACK_MODE_DCX |
           TRACK_MODE_EXT)) {
...
void TrackManager::Setup(const FSH *shieldname, MotorDriver *track0,
                         MotorDriver *track1, MotorDriver *track2,
                         MotorDriver *track3, MotorDriver *track4,
                         MotorDriver *track5, MotorDriver *track6,
                         MotorDriver *track7) {
  addTrack(0, track0);
  addTrack(1, track1);
  addTrack(2, track2);
  ...
  // Default the first 2 tracks (which may be null) and perform HA waveform
  // check.
...
void TrackManager::setDCSignal(int16_t cab, byte speedbyte) {
  FOR_EACH_TRACK(t) {
    ...
    else if (track[t]->getMode() == TRACK_MODE_DCX)
      track[t]->setDCSignal(speedbyte ^ 128);
  }
}

bool TrackManager::setTrackMode(byte trackToSet, TRACK_MODE mode,
                                int16_t dcAddr) {
  ...
  if (false) {
    ...
    track[trackToSet]->makeProgTrack(
        true); // set for prog track special handling
  } else {
    track[trackToSet]->makeProgTrack(
        false); // only the prog track knows it's type
  }
  ...
  FOR_EACH_TRACK(t) {
    ...
    if (track[t]->getMode() == TRACK_MODE_DC ||
        track[t]->getMode() == TRACK_MODE_DCX) {
    ...
    } else if (track[t]->getMode() == TRACK_MODE_MAIN ||
               track[t]->getMode() == TRACK_MODE_PROG) {
      track[t]->trackPWM =
          track[t]->isPWMCapable(); // trackPWM is still a guess here
  ...
  if (!canDo) {
    ...
    DCCTimer::clearPWM(); // has to be AFTER trackPWM changes because if
                          // trackPWM==true this is undone for  that track
  }
#else
  ...
  track[trackToSet]->setPower((mode == TRACK_MODE_MAIN ||
                               mode == TRACK_MODE_DC ||
                               mode == TRACK_MODE_DCX || mode == TRACK_MODE_EXT)
                                  ? mainPowerGuess
                                  : POWERMODE::OFF);
...
bool TrackManager::parseJ(Print *stream, int16_t params, int16_t p[]) {
  ...
  if (params == 2 && (p[1] == HASH_KEYWORD_OFF ||
                      p[1] == HASH_KEYWORD_NONE)) // <= id OFF> <= id NONE>
...
void TrackManager::loop() {
...
#endif
  bool dontLimitProg =
      DCCACK::isActive() || progTrackSyncMain || progTrackBoosted;
  nextCycleTrack++;
  ...
  bool useProgLimit = dontLimitProg
                          ? false
                          : track[nextCycleTrack]->getMode() == TRACK_MODE_PROG;
...
void TrackManager::reportObsoleteCurrent(Print *stream) {
  ...
  StringFormatter::send(stream, F("<c CurrentMAIN %d C Milli 0 %d 1 %d>\n"),
                        track[0]->raw2mA(track[0]->getCurrentRaw(false)),
...
void TrackManager::reportCurrent(Print *stream) {
  ...
  FOR_EACH_TRACK(t) {
    StringFormatter::send(
        stream, F(" %d"),
        (track[t]->getPower() == POWERMODE::OVERLOAD)
            ? -1
...
void TrackManager::setJoin(bool joined) {
  ...
  if (joined) {
    ...
    if (tempProgTrack != MAX_TRACKS + 1) {
      ...
      POWERMODE tPTmode =
          track[tempProgTrack]
              ->getPower(); // get current power status of this track
      setTrackMode(tempProgTrack, TRACK_MODE_PROG);
      track[tempProgTrack]->setPower(
          tPTmode); // set track status as it was before
