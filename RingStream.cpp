...
// Ideally, I would prefer to override the Print:print(_FlashStringHelper)
// function but the library authors omitted to make this virtual. Therefore we
// obveride the only other simple function that has no side effects in order
// that StringFormatter can recognise a RingStream and call its printFlash()
// directly.
int RingStream::availableForWrite() { return THIS_IS_A_RINGSTREAM; }
...
size_t RingStream::printFlash(const FSH *flashBuffer) {
  ...
  uintptr_t iFlash =
      reinterpret_cast<uintptr_t>(flash); // expect size match with pointer
...
int RingStream::count() { return (readRawByte() << 8) | readRawByte(); }
...
void RingStream::mark(uint8_t b) {
  // DIAG(F("RS mark client %d at %d core %d"), b, _pos_write,
  // xPortGetCoreID());
  _ringClient = b;
  _mark = _pos_write;
  write(b);          // client id
...
uint8_t RingStream::peekTargetMark() { return _ringClient; }

void RingStream::info() {
  DIAG(F("Info len=%d count=%d pr=%d pw=%d m=%d"), _len, _count, _pos_read,
       _pos_write, _mark);
}
...
bool RingStream::commit() {
  ...
  if (_count == 0) {
    // DIAG(F("RS commit count=0 rewind back to %d core %d"), _mark,
    // xPortGetCoreID());
