...
#define ATOMIC_BLOCK(x)                                                        \
  for (bool _int_saved                                                         \
       __attribute__((__cleanup__(_conditionalEnableInterrupts))) =            \
           _getInterruptState(),                                               \
       _ToDo = _deferInterrupts();                                             \
       _ToDo; _ToDo = 0)
...
#else
#warning                                                                       \
    "ATOMIC_BLOCK() not defined for this target type, I2C interrupts disabled"
...
// This module is only compiled if I2C_USE_WIRE is not defined, so undefine it
// here to get intellisense to work correctly.
      ...
      if (muxNumber != I2CMux_None) {
        ...
        muxData[0] = (subBus == SubBus_All)    ? 0xff
                     : (subBus == SubBus_None) ? 0x00
                                               ...
                                               0x04 | subBus; // NB Only 2 or 4
                                                              // subbuses
                                                              // respectively
#else
                                               // Default behaviour for most
                                               // MUXs is to use a mask with a
                                               // bit set for the subBus to be
                                               // enabled
                         1 << subBus;
#endif
 ...
 ***************************************************************************/
uint8_t I2CManagerClass::write(I2CAddress i2cAddress,
...
/***************************************************************************
 *  Initiate a write from PROGMEM (flash) to an I2C device (non-blocking
 *operation)
 ***************************************************************************/
uint8_t I2CManagerClass::write_P(I2CAddress i2cAddress,
                                 const uint8_t *writeBuffer, uint8_t writeLen,
                                 I2CRB *req) {
  // Make sure previous request has completed.
  req->wait();
 ...
 ***************************************************************************/
uint8_t I2CManagerClass::read(I2CAddress i2cAddress, uint8_t *readBuffer,
                              uint8_t readLen, const uint8_t *writeBuffer,
                              uint8_t writeLen, I2CRB *req) {
  // Make sure previous request has completed.
  req->wait();
 ...
 ***************************************************************************/
void I2CManagerClass::setTimeout(unsigned long value) { _timeout = value; };
...
void I2CManagerClass::checkForTimeout() {
  ATOMIC_BLOCK() {
    I2CRB *t = queueHead;
    if (state == I2C_STATE_ACTIVE && t != 0 && t == currentRequest &&
        _timeout > 0) {
      // Check for timeout
      ...
      if (elapsed > _timeout) {
        ...
        // If SDA is stuck low, issue up to 9 clock pulses to attempt to free
        // it.
...
void I2CManagerClass::handleInterrupt() {
  ...
  // and state isn't active then state contains the completion status of the
  // request.
  if (state == I2C_STATE_COMPLETED && currentRequest != NULL) {
    ...
    if (completionStatus == I2C_STATUS_OK || ++retryCounter > MAX_I2C_RETRIES ||
      ...
      // Status is OK, or has failed and retry count exceeded, or retries
      // disabled.
#if defined(I2C_EXTENDED_ADDRESS)
      if (muxPhase == MuxPhase_PROLOG) {
        ...
        if (_muxCount == 1) {
          ...
          deviceAddress =
              I2C_MUX_BASE_ADDRESS + currentRequest->i2cAddress.muxNumber();
          muxData[0] = 0x00;
          sendBuffer = &muxData[0];
          bytesToSend = 1;
          bytesToReceive = 0;
