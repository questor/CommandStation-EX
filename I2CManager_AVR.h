...
void I2CManagerClass::I2C_handleInterrupt() {
  ...
  case TWI_MTX_ADR_ACK:  // SLA+W has been transmitted and ACK received
    if (bytesToSend) {   // Send first.
      ...
      // Don't need to wait for stop, as the interface won't send the start
      // until any in-progress stop condition from previous interrupts has been
      // sent.
      TWCR = (1 << TWEN) | ENABLE_TWI_INTERRUPT | (1 << TWINT) |
             (1 << TWSTA); // Send Start
    } else {
      ...
      TWCR =
          (1 << TWEN) | (1 << TWINT) | (1 << TWEA) | (1 << TWSTO); // Send Stop
  ...
  case TWI_MRX_ADR_ACK: // SLA+R has been sent and ACK received
    if (bytesToReceive <= 1) {
      TWCR = (1 << TWEN) | ENABLE_TWI_INTERRUPT |
             (1 << TWINT); // Send NACK after next reception
    } else {
      // send ack
      TWCR = (1 << TWEN) | ENABLE_TWI_INTERRUPT | (1 << TWINT) | (1 << TWEA);
    }
...
#if defined(I2C_USE_INTERRUPTS)
ISR(TWI_vect) { I2CManager.handleInterrupt(); }
#endif
