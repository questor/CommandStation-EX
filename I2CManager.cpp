...
void I2CManagerClass::begin(void) {
  if (!_beginCompleted) {
    ...
    // Check for short-circuit or floating lines (no pull-up) on I2C before
    // enabling I2C
    ...
    for (uint8_t muxNo = I2CMux_0; muxNo <= I2CMux_7; muxNo++) {
      if (I2CManager.muxSelectSubBus({(I2CMux)muxNo, SubBus_None}) ==
          I2C_STATUS_OK)
        _muxCount++;
    }
#endif
    ...
    for (uint8_t addr = 0x08; addr < 0x78; addr++) {
      if (exists(addr)) {
        found = true;
        DIAG(F("I2C Device found at 0x%x, %S?"), addr,
             guessI2CDeviceType(addr));
      }
    }
    ...
    for (uint8_t muxNo = I2CMux_0; muxNo <= I2CMux_7; muxNo++) {
      ...
      if (exists(muxAddr)) {
        ...
        // Deselect all subBuses for this mux.  Otherwise its devices will
        // continue to respond when other muxes are being probed.
        I2CManager.muxSelectSubBus(
            {(I2CMux)muxNo, SubBus_None}); // Deselect Mux
      }
    }
#endif
    if (!found)
 ...
 ***************************************************************************/
uint8_t I2CManagerClass::write_P(I2CAddress i2cAddress, const uint8_t *data,
                                 uint8_t dataLen) {
  I2CRB req;
...
/***************************************************************************
 *  Initiate a write (optional) followed by a read from the I2C device (blocking
 *operation)
 ***************************************************************************/
uint8_t I2CManagerClass::read(I2CAddress i2cAddress, uint8_t *readBuffer,
  ...
  I2CRB req;
  uint8_t status =
      read(i2cAddress, readBuffer, readLen, writeBuffer, writeLen, &req);
  return finishRB(&req, status);
}
 ...
 *(blocking operation)
 ***************************************************************************/
uint8_t I2CManagerClass::read(I2CAddress address, uint8_t readBuffer[],
                              uint8_t readSize, uint8_t writeSize, ...) {
  va_list args;
 ...
 ***************************************************************************/
void I2CRB::setReadParams(I2CAddress i2cAddress, uint8_t *readBuffer,
                          uint8_t readLen) {
...
void I2CRB::setRequestParams(I2CAddress i2cAddress, uint8_t *readBuffer,
                             uint8_t readLen, const uint8_t *writeBuffer,
                             uint8_t writeLen) {
  this->i2cAddress = i2cAddress;
...
void I2CRB::setWriteParams(I2CAddress i2cAddress, const uint8_t *writeBuffer,
                           uint8_t writeLen) {
  this->i2cAddress = i2cAddress;
  this->writeBuffer = writeBuffer;
  this->writeLen = writeLen;
