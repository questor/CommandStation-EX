...
void SERCOM3_Handler() { I2CManager.handleInterrupt(); }
#endif
...
void I2CManagerClass::I2C_setClock(uint32_t i2cClockSpeed) {
  ...
  if (i2cClockSpeed < 200000L) {
    i2cClockSpeed =
        100000L; // NB: this overrides a "force clock" of lower than 100KHz!
    t_rise = 1000;
  ...
  s->I2CM.BAUD.bit.BAUD = SystemCoreClock / (2 * i2cClockSpeed) - 5 -
                          (((SystemCoreClock / 1000000) * t_rise) / (2 * 1000));
...
void I2CManagerClass::I2C_init() {
  // Setting clock
  GCLK->CLKCTRL.reg =
      GCLK_CLKCTRL_ID(GCM_SERCOM3_CORE) | // Generic Clock 0 (SERCOM3)
  ...
  NVIC_SetPriority(SERCOM3_IRQn,
                   SERCOM_NVIC_PRIORITY); // Match default SERCOM priorities
  //  NVIC_SetPriority (SERCOM3_IRQn, 0);  // Set highest priority

  // Enable all interrupts
  s->I2CM.INTENSET.reg = SERCOM_I2CM_INTENSET_MB | SERCOM_I2CM_INTENSET_SB |
  ...
  // Set SDA/SCL pins as outputs and enable pullups, at present we assume these
  // are the default ones for SERCOM3 (see assumption above)
  pinPeripheral(PIN_WIRE_SDA, g_APinDescription[PIN_WIRE_SDA].ulPinType);
  pinPeripheral(PIN_WIRE_SCL, g_APinDescription[PIN_WIRE_SCL].ulPinType);

  // Enable the SCL and SDA pins on the sercom: includes increased driver
  // strength, pull-up resistors and pin multiplexer
  PORT->Group[g_APinDescription[PIN_WIRE_SCL].ulPort]
      .PINCFG[g_APinDescription[PIN_WIRE_SCL].ulPin]
      .reg = PORT_PINCFG_DRVSTR | PORT_PINCFG_PULLEN | PORT_PINCFG_PMUXEN;
  PORT->Group[g_APinDescription[PIN_WIRE_SDA].ulPort]
      .PINCFG[g_APinDescription[PIN_WIRE_SDA].ulPin]
      .reg = PORT_PINCFG_DRVSTR | PORT_PINCFG_PULLEN | PORT_PINCFG_PMUXEN;
}
