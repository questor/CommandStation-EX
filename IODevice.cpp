...
// Link to halSetup function.  If not defined, the function reference will be
// NULL.
...
// Create any standard device instances that may be required, such as the
// Arduino pins and PCA9685.
void IODevice::begin() {
  // Initialise the IO subsystem defaults
  ArduinoPins::create(2,
                      NUM_DIGITAL_PINS - 2); // Reserve pins for direct access

  // Call user's halSetup() function (if defined in the build in myHal.cpp).
  //  The contents will depend on the user's system hardware configuration.
  //  The myHal.cpp file is a standard C++ module so has access to all of the
  //  DCC++EX APIs.
  // This is done early so that the subsequent defaults will detect an overlap
  // and not create something that conflicts with the user's vpin definitions.
  if (halSetup)
    halSetup();
  ...
  if (checkNoOverlap(100, 16, 0x40)) {
    ...
    DIAG(F(
        "Default PCA9685 at I2C 0x40 disabled due to configured user device"));
  }
  if (checkNoOverlap(116, 16, 0x41)) {
    PCA9685::create(116, 16, 0x41);
  } else {
    DIAG(F(
        "Default PCA9685 at I2C 0x41 disabled due to configured user device"));
  }
  ...
  if (checkNoOverlap(164, 16, 0x20)) {
    ...
    DIAG(F(
        "Default MCP23017 at I2C 0x20 disabled due to configured user device"));
  }
  if (checkNoOverlap(180, 16, 0x21)) {
    MCP23017::create(180, 16, 0x21);
  } else {
    DIAG(F(
        "Default MCP23017 at I2C 0x21 disabled due to configured user device"));
  }
}
...
// Overarching static loop() method for the IODevice subsystem.  Works through
// the list of installed devices and calls their individual _loop() method.
// Devices may or may not implement this, but if they do it is useful for things
// like animations or flashing LEDs. The current value of micros() is passed as
// a parameter, so the called loop function doesn't need to invoke it.
void IODevice::loop() {
  unsigned long currentMicros = micros();
  ...
  do {
    ...
    if (_nextLoopDevice) {
      ...
      _nextLoopDevice = _nextLoopDevice->_nextDevice;
           ...
           lastLoopDevice); // Stop looking when we've done all.
  ...
  const unsigned long interval =
      (unsigned long)5 * 1000 * 1000; // 5 seconds in microsec
  ...
  if (diagMicros - lastOutputTime > interval) {
    if (lastOutputTime > 0)
      DIAG(F("Loop Total:%lus (%lus max) HAL:%lus (%lus max)"), total / count,
           maxElapsed, halTotal / count, maxHalElapsed);
...
bool IODevice::exists(VPIN vpin) { return findDevice(vpin) != NULL; }

// check whether the pin supports notification.  If so, then regular _read calls
// are not required.
...
void IODevice::_display() {
  DIAG(F("Unknown device Vpins:%u-%u %S"), (int)_firstVpin,
       (int)_firstVpin + _nPins - 1,
       _deviceState == DEVSTATE_FAILED ? F("OFFLINE") : F(""));
}

// Find device associated with nominated Vpin and pass configuration values on
// to it.
//   Return false if not found.
bool IODevice::configure(VPIN vpin, ConfigTypeEnum configType, int paramCount,
                         int params[]) {
  IODevice *dev = findDevice(vpin);
  if (dev)
...
// Write value to virtual pin(s).  If multiple devices are allocated the same
// pin
...
//
void IODevice::writeAnalogue(VPIN vpin, int value, uint8_t param1,
                             uint16_t param2) {
  IODevice *dev = findDevice(vpin);
  if (dev) {
...
// isBusy, when called for a device pin is always a digital output or analogue
// output,
//  returns input feedback state of the pin, i.e. whether the pin is busy
//  performing an animation or fade over a period of time.
...
// Private helper function to locate a device by VPIN.  Returns NULL if not
// found.
//  This is performance-critical, so minimises the calculation and function
//  calls necessary.
...
bool IODevice::checkNoOverlap(VPIN firstPin, uint8_t nPins,
  ...
  for (IODevice *dev = _firstDevice; dev != 0; dev = dev->_nextDevice) {
    ...
    if (i2cAddress && dev->_I2CAddress == i2cAddress) {
      DIAG(F("WARNING HAL Overlap. i2c Addr %s ignored."),
           i2cAddress.toString());
      return false;
    }
  }
...
// Chain of callback blocks (identifying registered callback functions for state
// changes)
...
// Minimal implementations of public HAL interface, to support Arduino pin I/O
// and nothing more.

void IODevice::begin() { DIAG(F("NO HAL CONFIGURED!")); }
bool IODevice::configure(VPIN pin, ConfigTypeEnum configType, int nParams,
                         int p[]) {
...
int IODevice::readAnalogue(VPIN vpin) { return ADCee::read(vpin); }
int IODevice::configureAnalogIn(VPIN vpin) { return ADCee::init(vpin); }
void IODevice::loop() {}
void IODevice::DumpAll() { DIAG(F("NO HAL CONFIGURED!")); }
...
// Chain of callback blocks (identifying registered callback functions for state
// changes) Not used in IO_NO_HAL but must be declared.
IONotifyCallback *IONotifyCallback::first = 0;
...
// Device-specific pin configuration.  Configure should be called infrequently
// so simplify code by using the standard pinMode function.
bool ArduinoPins::_configure(VPIN vpin, ConfigTypeEnum configType,
                             int paramCount, int params[]) {
