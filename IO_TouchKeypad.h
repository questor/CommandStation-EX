 ...
 * The pins clockPin and dataPin must be local pins, not external (GPIO
 * Expander) pins.
 *
 * To use,
 ...
 * the pins OUT1 to OUT8 are not used but all sixteen touch pads are
 * operational.
 *
 * TODO: Allow a list of datapins to be provided so that multiple keypads can
 * be read simultaneously by the one device driver and the one shared clock
 * signal. As it stands, we can configure multiple driver instances, one for
 * each keypad, and it will work fine.  The clock will be driven to all devices
 * but only one driver will be reading the responses from its corresponding
 * device at a time.
 */
...
protected:
  ...
  // Display information about the device, and perhaps its current condition
  // (e.g. active, disabled etc).
  void _display() {
    DIAG(F("TouchKeypad Configured on Vpins:%u-%u SCL=%d SDO=%d"),
         (int)_firstVpin, (int)_firstVpin + _nPins - 1, _clockPin, _dataPin);
  }
};
