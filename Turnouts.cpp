...
/* static */ bool Turnout::remove(uint16_t id) {
  ...
  for (tt = _firstTurnout; tt != NULL && tt->_turnoutData.id != id;
       pp = tt, tt = tt->_nextTurnout) {
  }
  if (tt == NULL)
    return false;
...
//  type should be placed in the virtual function setClosedInternal(bool) which
//  is called from here.
...
#ifndef DISABLE_EEPROM
    // Write byte containing new closed/thrown state to EEPROM if required. Note
    // that eepromAddress is always zero for LCN turnouts.
...
/* static */ Turnout *Turnout::loadTurnout() {
  ...
  int eepromAddress =
      EEStore::pointer() +
      offsetof(struct TurnoutData,
               flags); // Address of byte containing the closed flag.
  ...
  if (tt) {
    // Save EEPROM address in object.  Note that LCN turnouts always have
...
// Private Constructor
ServoTurnout::ServoTurnout(uint16_t id, VPIN vpin, uint16_t thrownPosition,
                           uint16_t closedPosition, uint8_t profile,
                           bool closed)
...
// Create function
/* static */ Turnout *ServoTurnout::create(uint16_t id, VPIN vpin,
                                           uint16_t thrownPosition,
                                           uint16_t closedPosition,
                                           uint8_t profile, bool closed) {
#ifndef IO_NO_HAL
  Turnout *tt = get(id);
  if (tt) {
    ...
    if (tt->isType(TURNOUT_SERVO)) {
      ...
      //  configured a Turnout, we should ensure that the SET() RESET() and
      //  other commands that use write() behave consistently with the turnout
      //  commands.
      IODevice::configureServo(vpin, thrownPosition, closedPosition, profile, 0,
                               closed);

      // Set position directly to specified position - we don't know where it is
      // moving from.
      IODevice::writeAnalogue(vpin, closed ? closedPosition : thrownPosition,
                              PCA9685::Instant);
  ...
  tt = (Turnout *)new ServoTurnout(id, vpin, thrownPosition, closedPosition,
                                   profile, closed);
  DIAG(F("Turnout 0x%x size %d size %d"), tt, sizeof(Turnout),
       sizeof(struct TurnoutData));
  IODevice::writeAnalogue(vpin, closed ? closedPosition : thrownPosition,
                          PCA9685::Instant);
  return tt;
#else
...
// Load a Servo turnout definition from EEPROM.  The common Turnout data has
...
Turnout *ServoTurnout::load(struct TurnoutData *turnoutData) {
  ...
  // Create new object
  Turnout *tt = ServoTurnout::create(
      turnoutData->id, servoTurnoutData.vpin, servoTurnoutData.thrownPosition,
      servoTurnoutData.closedPosition, servoTurnoutData.profile,
      turnoutData->closed);
  return tt;
#else
  (void)turnoutData;
...
void ServoTurnout::print(Print *stream) {
  StringFormatter::send(stream, F("<H %d SERVO %d %d %d %d %d>\n"),
                        ...
                        _servoTurnoutData.thrownPosition,
                        _servoTurnoutData.closedPosition,
                        _servoTurnoutData.profile, !_turnoutData.closed);
}
...
bool ServoTurnout::setClosedInternal(bool close) {
  ...
  IODevice::writeAnalogue(_servoTurnoutData.vpin,
                          close ? _servoTurnoutData.closedPosition
                                : _servoTurnoutData.thrownPosition,
...
// Create function
/* static */ Turnout *DCCTurnout::create(uint16_t id, uint16_t add,
                                         uint8_t subAdd) {
  Turnout *tt = get(id);
  if (tt) {
    // Object already exists, check if it is usable
...
// Load a DCC turnout definition from EEPROM.  The common Turnout data has
// already been read at this point.
/* static */ Turnout *DCCTurnout::load(struct TurnoutData *turnoutData) {
  ...
  // Create new object
  DCCTurnout *tt = new DCCTurnout(turnoutData->id, dccTurnoutData.address,
                                  dccTurnoutData.subAddress);
...
void DCCTurnout::print(Print *stream) {
  StringFormatter::send(stream, F("<H %d DCC %d %d %d>\n"), _turnoutData.id,
                        _dccTurnoutData.address, _dccTurnoutData.subAddress,
                        !_turnoutData.closed);
  // Also report using classic DCC++ syntax for DCC accessory turnouts, since
  // JMRI expects this.
...
bool DCCTurnout::setClosedInternal(bool close) {
  ...
  DCC::setAccessory(_dccTurnoutData.address, _dccTurnoutData.subAddress,
                    close ^ !rcn213Compliant);
  return true;
}
...
// Load a VPIN turnout definition from EEPROM.  The common Turnout data has
// already been read at this point.
/* static */ Turnout *VpinTurnout::load(struct TurnoutData *turnoutData) {
  ...
  // Create new object
  VpinTurnout *tt = new VpinTurnout(turnoutData->id, vpinTurnoutData.vpin,
                                    turnoutData->closed);
...
bool LCNTurnout::setClosedInternal(bool close) {
  ...
  // The _turnoutData.closed flag should be updated by a message from the LCN
  // master. but in this implementation it is updated in setClosedStateOnly()
  // instead. If the LCN master updates this, setClosedStateOnly() and all
  // setClosedInternal() have to be updated accordingly so that the closed flag
  // is only set once.
  return true;
}
