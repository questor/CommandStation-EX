...
protected:
  // Constructor.  This should initialise variables etc. but not call other
  // objects yet (e.g. Serial, I2CManager, and other parts of the CS
  // functionality). defer those until the _begin() function.  The 'addDevice'
  // call is required unless the device is not to be added (e.g. because of
  // incorrect parameters).
  IO_ExampleSerial(VPIN firstVpin, int nPins, HardwareSerial *serial,
                   unsigned long baud) {
    ...
    if (_serial->available()) {
      ...
      case 3: // reading reading second parameter (value)
        if (isdigit(c))
          ...
          DIAG(F("ExampleSerial Received command, p1=%d, p2=%d"), _inputIndex,
               _inputValue);
  ...
  // (e.g. active, disabled etc). Here we display the current values held for
  // the pins.
  void _display() {
