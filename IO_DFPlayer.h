...
/*
 * DFPlayer is an MP3 player module with an SD card holder.  It also has an
 integrated
 ...
 *   and Serialn is the name of the Serial port connected to the DFPlayer (e.g.
 Serial1).
 *
 * Example:
 ...
 *
 * Writing an analogue value 1-2999 to the first pin (3500) will play the
 numbered file from the
 * SD card; e.g. a value of 1 will play the first file, 2 for the second file
 etc.
 * Writing an analogue value 0 to the first pin (3500) will stop the file
 playing;
 * Writing an analogue value 0-30 to the second pin (3501) will set the volume;
 * Writing a digital value of 1 to a pin will play the file corresponding to
 that pin, e.g. the first file will be played by setting pin 3500, the second by
 setting pin 3501 etc.;
 * Writing a digital value of 0 to any pin will stop the player;
 * Reading a digital value from any pin will return true(1) if the player is
 playing, false(0) otherwise.
 *
 ...
 *   WAITFOR(3500)  -- wait for the file currently being played by the player to
 complete
 *   SERVO(3500,2,Instant)  -- plays file 2 at current volume
 *   SERVO(3501,20,Instant)   -- Sets the volume to 20
 *
 * NB The DFPlayer's serial lines are not 5V safe, so connecting the Arduino TX
 directly
 * to the DFPlayer's RX terminal will cause lots of noise over the speaker, or
 worse.
 * A 1k resistor in series with the module's RX terminal will alleviate this.
 *
 * Files on the SD card are numbered according to their order in the directory
 on the
 * card (as listed by the DIR command in Windows).  This may not match the order
 of the files
 * as displayed by Windows File Manager, which sorts the file names.  It is
 suggested that
 * files be copied into an empty SDcard in the desired order, one at a time.
 *
 * The driver now polls the device for its current status every second.  Should
 the device
 * fail to respond it will be marked off-line and its busy indicator cleared, to
 avoid
...
protected:
  ...
  DFPlayer(VPIN firstVpin, int nPins, HardwareSerial &serial)
      : IODevice(firstVpin, nPins), _serial(&serial) {
    addDevice(this);
  }
  ...
  void _loop(unsigned long currentMicros) override {
    ...
    // Check if a command sent to device has timed out.  Allow 0.5 second for
    // response
  ...
  // Check for incoming data on _serial, and update busy flag and other state
  // accordingly
  ...
  void processOutgoing(unsigned long currentMicros) {
    ...
    // When two commands are sent in quick succession, the device will often
    // fail to execute one.  Testing has indicated that a delay of 100ms or more
    // is required between successive commands to get reliable operation. If
    // 100ms has elapsed since the last thing sent, then check if there's some
    // output to do.
    if (((int32_t)currentMicros - _commandSendTime) > 100000) {
      if (_currentVolume > _requestedVolumeLevel) {
        ...
        if (!_awaitingResponse) {
          _timeoutTime = currentMicros +
                         5000000UL; // Timeout if no response within 5 seconds
          _awaitingResponse = true;
        }
      }
    }
  ...
  // Write with value 1 starts playing a song.  The relative pin number is the
  // file number. Write with value 0 stops playing.
...
// Value 0, stop playing
  ...
  // WriteAnalogue on first pin uses the nominated value as a file number to
  // start playing, if file number > 0. Volume may be specified as second
  // parameter to writeAnalogue. If value is zero, the player stops playing.
  // WriteAnalogue on second pin sets the output volume.
  //
  void _writeAnalogue(VPIN vpin, int value, uint8_t volume = 0,
                      uint16_t = 0) override {
    if (_deviceState == DEVSTATE_FAILED)
      return;
  ...
  void _display() override {
    DIAG(F("DFPlayer Configured on Vpins:%u-%u %S"), _firstVpin,
         _firstVpin + _nPins - 1,
