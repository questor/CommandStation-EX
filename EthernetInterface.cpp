...
EthernetInterface::EthernetInterface() {
  ...
  while ((millis() - startmilli) <
         5500) { // Loop to give time to check for cable connection
...
/**
 * @brief Checks ethernet link cable status and detects when it connects /
 * disconnects
 *
...
bool EthernetInterface::checkLink() {
  if (Ethernet.linkStatus() !=
      LinkOFF) { // check for not linkOFF instead of linkON as the W5100 does
                 // return LinkUnknown
...
#endif
      IPAddress ip =
          Ethernet.localIP(); // look what IP was obtained (dynamic or static)
      server = new EthernetServer(
          IP_PORT); // Ethernet Server listening on default port IP_PORT
      server->begin();
...
void EthernetInterface::loop2() {
  if (!outboundRing) { // no idea to call loop2() if we can't handle outgoing
                       // data in it
    if (Diag::ETHERNET)
      DIAG(F("No outboundRing"));
    return;
  }
  ...
  for (byte socket = 0; socket < MAX_SOCK_NUM; socket++) {
    if (clients[socket]) {
      ...
      if (available > 0) {
        ...
        return; // limit the amount of processing that takes place within 1
                // loop() cycle.
      }
    }
  }
  ...
  if (socketOut >= MAX_SOCK_NUM) {
    ...
    for (; count > 0; count--)
      clients[socketOut].write(outboundRing->read());
