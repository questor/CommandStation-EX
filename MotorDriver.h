...
// use powers of two so we can do logical and/or on the track modes in if
// clauses.
...
#ifndef HAVE_PORTA
#define HAVE_PORTA(X)                                                          \
  byte TOKENPASTE2(Unique_, __LINE__) __attribute__((unused)) = 0
#endif
#ifndef HAVE_PORTB
#define HAVE_PORTB(X)                                                          \
  byte TOKENPASTE2(Unique_, __LINE__) __attribute__((unused)) = 0
...
class MotorDriver {
public:
  MotorDriver(int16_t power_pin, byte signal_pin, byte signal_pin2,
              int16_t brake_pin, byte current_pin, float senseFactor,
  ...
  inline int getRawCurrentTripValue() { return rawCurrentTripValue; }
  bool isPWMCapable();
  bool canMeasureCurrent();
  bool trackPWM =
      false; // this track uses PWM timer to generate the DCC waveform
  bool commonFaultPin = false; // This is a stupid motor shield which has only a
                               // common fault pin for both outputs
  inline byte setCommonFaultPin() { return commonFaultPin = true; }
  inline byte getFaultPin() { return faultPin; }
  inline void
  makeProgTrack(bool on) { // let this output know it's a prog track.
    isProgTrack = on;
  }
  void checkPowerOverload(bool useProgLimit, byte trackno);
  inline void setTrackLetter(char c) { trackLetter = c; };
  ...
  inline unsigned long microsSinceLastPowerChange(POWERMODE mode) {
    ...
    if (diff >
        (1UL << (7 * sizeof(unsigned long)))) // 2^(4*7)us = 268.4 seconds
...
#endif
  inline void setMode(TRACK_MODE m) { trackMode = m; };
  inline TRACK_MODE getMode() { return trackMode; };
...
private:
  ...
  TRACK_MODE trackMode =
      TRACK_MODE_NONE; // we assume track not assigned at startup
};
#endif
