...
When determining a new OPCODE for a new feature, refer to this list as the
source of truth.
...
// Non-DCC things like turnouts, pins and sensors are handled in additional JMRI
// interface classes.

int16_t DCCEXParser::splitValues(int16_t result[MAX_COMMAND_PARAMS],
                                 const byte *cmd, bool usehex) {
  byte state = 1;
  ...
  while (parameterCount < MAX_COMMAND_PARAMS) {
    ...
    case 3: // building a parameter
      ...
      if (hot == '_' || (hot >= 'A' && hot <= 'Z')) {
        // Since JMRI got modified to send keywords in some rare cases, we need
        // this Super Kluge to turn keywords into a hash value that can be
        // recognised later
...
extern __attribute__((weak)) void myFilter(Print *stream, byte &opcode,
                                           byte &paramCount, int16_t p[]);
FILTER_CALLBACK DCCEXParser::filterCallback = myFilter;
FILTER_CALLBACK DCCEXParser::filterRMFTCallback = 0;
AT_COMMAND_CALLBACK DCCEXParser::atCommandCallback = 0;

// deprecated
void DCCEXParser::setFilter(FILTER_CALLBACK filter) { filterCallback = filter; }
...
void DCCEXParser::parseOne(Print *stream, byte *com, RingStream *ringStream) {
  ...
  // Functions return from this switch if complete, break from switch implies
  // error <X> to send
  switch (opcode) {
  case '\0':
  ...
  case 'a': // ACCESSORY <a ADDRESS SUBADDRESS ACTIVATE [ONOFF]> or <a
            // LINEARADDRESS ACTIVATE>
  {
    int address;
    byte subaddress;
    byte activep;
    ...
    if (((address & 0x01FF) != address) // invalid address (limit 9 bits)
      ...
      break;
      // Honour the configuration option (config.h) which allows the <a> command
      // to be reversed
  ...
  case 'P': // WRITE TRANSPARENT DCC PACKET PROG <P REG X1 ... X9>
      ...
      break;
    {
      ...
      (opcode == 'M' ? DCCWaveform::mainTrack : DCCWaveform::progTrack)
          .schedulePacket(packet, params, 3);
    }
    return;
  ...
  case 'W': // WRITE CV ON PROG <W CV VALUE CALLBACKNUM CALLBACKSUB>
    if (!stashCallback(stream, p, ringStream))
      break;
    if (params == 1) // <W id> Write new loco id (clearing consist and managing
                     // short/long)
      DCC::setLocoId(p[0], callback_Wloco);
    else if (params ==
             4) // WRITE CV ON PROG <W CV VALUE [CALLBACKNUM] [CALLBACKSUB]>
  ...
  case '0': // POWEROFF <0 [MAIN | PROG] >
  {
    ...
    if (prog) {
      TrackManager::progTrackBoosted =
          false; // Prog track boost mode will not outlive prog track off
  ...
  case '!': // ESTOP ALL  <!>
    DCC::setThrottle(
        0, 1,
        1); // this broadcasts speed 1(estop) and sets all reminders to speed 1.
    return;
  ...
  case 's': // <s>
    StringFormatter::send(stream, F("<iDCC-EX V-%S / %S / %S G-%S>\n"),
                          ...
                          DCC::getMotorShieldName(), F(GITHUB_SHA));
    CommandDistributor::broadcastPower(); // <s> is the only "get power status"
                                          // command we have
  ...
  case 'J': // throttle info access
  {
    ...
    case HASH_KEYWORD_A: // <JA> returns automations/routes
      ...
      if (params == 1) { // <JA>
        ...
        StringFormatter::send(stream, F(" %d %c \"%S\""), id,
#ifdef EXRAIL_ACTIVE
    ...
    case HASH_KEYWORD_R: // <JR> returns rosters
      ...
      if (params == 1) {
        ...
        if (!functionNames)
          functionNames = F("");
        StringFormatter::send(stream, F(" %d \"%S\" \"%S\""), id, rosterName,
                              functionNames);
      }
#endif
      StringFormatter::send(stream, F(">\n"));
  ...
  case 0: // <Z> list Output definitions
  {
    ...
    for (Output *tt = Output::firstOutput; tt != NULL; tt = tt->nextOutput) {
      ...
      StringFormatter::send(stream, F("<Y %d %d %d %d>\n"), tt->data.id,
                            tt->data.pin, tt->data.flags, tt->data.active);
    }
    return gotone;
  }
  default:
...
bool DCCEXParser::parseT(Print *stream, int16_t params, int16_t p[]) {
  ...
  default: // Anything else is some kind of turnout create function.
    if (params == 6 && p[1] == HASH_KEYWORD_SERVO) { // <T id SERVO n n n n>
      if (!ServoTurnout::create(p[0], (VPIN)p[2], (uint16_t)p[3],
                                (uint16_t)p[4], (uint8_t)p[5]))
        return false;
    } else if (params == 3 && p[1] == HASH_KEYWORD_VPIN) { // <T id VPIN n>
      if (!VpinTurnout::create(p[0], p[2]))
        return false;
    } else if (params >= 3 && p[1] == HASH_KEYWORD_DCC) {
      // <T id DCC addr subadd>   0<=addr<=511, 0<=subadd<=3 (like <a>
      // command).<T>
      if (params == 4 && p[2] >= 0 && p[2] < 512 && p[3] >= 0 &&
          p[3] < 4) { // <T id DCC n m>
        if (!DCCTurnout::create(p[0], p[2], p[3]))
          return false;
      } else if (params == 3 && p[2] > 0 &&
                 p[2] <= 512 * 4) { // <T id DCC nn>, 1<=nn<=2048
      ...
      if (!ServoTurnout::create(p[0], (VPIN)p[1], (uint16_t)p[2],
                                (uint16_t)p[3], 1))
        return false;
    } else
      return false;
...
bool DCCEXParser::parseD(Print *stream, int16_t params, int16_t p[]) {
  ...
  bool onOff =
      (params > 0) &&
      (p[1] == 1 || p[1] == HASH_KEYWORD_ON); // dont care if other stuff or
                                              // missing... just means off
  switch (p[0]) {
  ...
  case HASH_KEYWORD_ACK: // <D ACK ON/OFF> <D ACK [LIMIT|MIN|MAX|RETRY] Value>
    if (params >= 3) {
      if (p[1] == HASH_KEYWORD_LIMIT) {
        ...
        if (p[2] > 255)
          p[2] = 3;
        LCD(0, F("Ack Retry=%d Sum=%d"), p[2],
            DCCACK::setAckRetry(p[2])); //   <D ACK RETRY 2>
      }
    } else {
