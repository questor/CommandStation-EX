...
private:
  ...
  const uint8_t PCA9685_FIRST_SERVO =
      0x06; /** low uint8_t first PWM register ON*/
  const uint8_t PCA9685_PRESCALE =
  ...
  const uint32_t FREQUENCY_OSCILLATOR =
      25000000; /** Accurate enough for our purposes  */
  const uint8_t PRESCALE_50HZ =
      (uint8_t)(((FREQUENCY_OSCILLATOR / (50.0 * 4096.0)) + 0.5) - 1);
  const uint32_t MAX_I2C_SPEED =
  ...
  PCA9685pwm(VPIN firstVpin, int nPins, I2CAddress i2cAddress,
    ...
    requestBlock.setWriteParams(_I2CAddress, outputBuffer,
                                sizeof(outputBuffer));
  }
  ...
  void _begin() override {
    ...
    if (I2CManager.exists(_I2CAddress)) {
      ...
      // In theory, we should wait 500us before sending any other commands to
      // each device, to allow the PWM oscillator to get running.  However, we
      // don't do any specific wait, as there's plenty of other stuff to do
      // before we will send a command.
#if defined(DIAG_IO)
      _display();
#endif
    } else
  ...
  //
  void _writeAnalogue(VPIN vpin, int value, uint8_t param1,
                      uint16_t param2) override {
    (void)param1;
    (void)param2; // suppress compiler warning
#ifdef DIAG_IO
    DIAG(F("PCA9685pwm WriteAnalogue VPIN:%u Value:%d %S"), vpin, value,
  ...
  void _display() override {
    DIAG(F("PCA9685pwm I2C:%s Configured on Vpins:%u-%u %S"),
         _I2CAddress.toString(), (int)_firstVpin, (int)_firstVpin + _nPins - 1,
         (_deviceState == DEVSTATE_FAILED) ? F("OFFLINE") : F(""));
  }

  // writeDevice (helper function) takes a pin in range 0 to _nPins-1 within the
  // device, and a value between 0 and 4095 for the PWM mark-to-period ratio,
  // with 4095 being 100%.
  void writeDevice(uint8_t pin, int value) {
#ifdef DIAG_IO
    DIAG(F("PCA9685pwm I2C:%s WriteDevice Pin:%d Value:%d"),
         _I2CAddress.toString(), pin, value);
#endif
    // Wait for previous request to complete
    uint8_t status = requestBlock.wait();
    if (status != I2C_STATUS_OK) {
      _deviceState = DEVSTATE_FAILED;
      DIAG(F("PCA9685pwm I2C:%s failed %S"), _I2CAddress.toString(),
