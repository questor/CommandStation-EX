...
// Uncomment following line to slow the scan cycle down to 1second ADC samples,
// with diagnostic output of scanned values.
...
/**********************************************************************************************
 * ADS111x class for I2C-connected analogue input modules ADS1113, ADS1114 and
 *ADS1115.
 *
 * ADS1113 and ADS1114 are restricted to 1 input.  ADS1115 has a multiplexer
 *which allows any of four input pins to be read by its ADC.
 *
 * The driver polls the device in accordance with the constant 'scanInterval'
 *below.  On first loop entry, the multiplexer is set to pin A0 and the ADC is
 *triggered.  On second and subsequent entries, the analogue value is read from
 *the conversion register and then the multiplexer and ADC are set up to read
 *the next pin.
 *
 ...
 *    Comparator off
 *    Gain FSR=6.144V
 * The gain means that the maximum input voltage of 5V (when Vss=5V) gives a
 *reading of 32767*(5.0/6.144) = 26666.
 *
 ...
 *
 * Note: The device is simple and does not need initial configuration, so it
 *should recover from temporary loss of communications or power.
 **********************************************************************************************/
...
private:
  ...
  void _loop(unsigned long currentMicros) override {
    ...
    if (status == I2C_STATUS_OK) {
      ...
      case STATE_STARTSCAN:
        // Configure ADC and multiplexer for next scan.  See ADS111x datasheet
        // for details of configuration register settings.
        _outBuffer[0] = 0x01; // Config register address
        _outBuffer[1] =
            0xC0 + (_currentPin << 4); // Trigger single-shot, channel n
      ...
      case STATE_STARTREAD:
        ...
        I2CManager.read(_I2CAddress, _inBuffer, 2, _outBuffer, 1,
                        &_i2crb); // Read register
        _nextState = STATE_GETVALUE;
        break;

      case STATE_GETVALUE:
        _value[_currentPin] =
            ((uint16_t)_inBuffer[0] << 8) + (uint16_t)_inBuffer[1];
      ...
      DIAG(F("ADS111x I2C:%s Error:%d %S"), _I2CAddress.toString(), status,
  ...
  void _display() override {
    DIAG(F("ADS111x I2C:%s Configured on Vpins:%u-%u %S"),
         _I2CAddress.toString(), _firstVpin, _firstVpin + _nPins - 1,
         _deviceState == DEVSTATE_FAILED ? F("OFFLINE") : F(""));
  }

// ADC conversion rate is 250SPS, or 4ms per conversion.  Set the period between
// updates to 10ms. This is enough to allow the conversion to reliably complete
// in time.
#ifndef IO_ANALOGUE_SLOW
  const unsigned long scanInterval =
      10000UL; // Period between successive ADC scans in microseconds.
#else
  const unsigned long scanInterval =
      1000000UL; // Period between successive ADC scans in microseconds.
#endif
  enum : uint8_t {
    STATE_STARTSCAN,
    STATE_STARTREAD,
