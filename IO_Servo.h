 ...
 * device.  The underlying device class is expected to accept writeAnalogue
 * calls which will normally cause some physical movement of something.  The
 * device may be a servo, a motor or some other kind of positioner, and the
 * something might be a turnout, a semaphore signal or something else.  One user
 * has used this capability for moving a figure along the platform on their
 * layout!
 *
 * Example of use:
 *    In myHal.cpp,
 ...
 * PWM commands to the corresponding PCA9685 driver pins 100-115.  The PCA9685
 * driver may be substituted with any other driver which provides analogue
 * output capability, e.g. EX-IOExpander devices, as long as they are capable of
 * interpreting the writeAnalogue() function calls.
 */
...
private:
  ...
  bool _configure(VPIN vpin, ConfigTypeEnum configType, int paramCount,
    ...
    if (paramCount != 5)
      return false;
#ifdef DIAG_IO
    DIAG(F("Servo: Configure VPIN:%u Apos:%d Ipos:%d Profile:%d Duration:%d "
           "state:%d"),
  ...
  Servo(VPIN firstVpin, int nPins, VPIN firstSlavePin = VPIN_NONE) {
    ...
    // To save RAM, space for servo configuration is not allocated unless a pin
    // is used. Initialise the pointers to NULL.
    ...
    if (!_slaveDevice) {
      DIAG(F("Servo: Slave device not found on Vpins %u-%u"), _firstSlavePin,
           _firstSlavePin + _nPins - 1);
    ...
    addDevice(this, _slaveDevice); // Link device ahead of slave device to
                                   // intercept requests
  }
  ...
  void _write(VPIN vpin, int value) override {
    ...
    if (s != NULL) {
      ...
      writeAnalogue(vpin, value ? s->activePosition : s->inactivePosition,
                    s->profile, s->duration);
    } else {
  ...
  // Device-specific writeAnalogue function, invoked from
  // IODevice::writeAnalogue(). Profile is as follows:
  //  Bit 7:     0=Set output to 0% to power off servo motor when finished
  //             1=Keep output at final position (better with LEDs, which will
  //             stay lit)
  ...
  void _writeAnalogue(VPIN vpin, int value, uint8_t profile,
    ...
    if (s == NULL) {
      ...
      s = _servoData[pin] =
          (struct ServoData *)calloc(sizeof(struct ServoData), 1);
      if (s == NULL)
      ...
      s->profile =
          Instant | NoPowerOff; // Use instant profile (but not this time)
    }

    // Animated profile.  Initiate the appropriate action.
    s->currentProfile = profile;
    uint8_t profileValue =
        profile & ~NoPowerOff; // Mask off 'don't-power-off' bit.
    s->numSteps =
        profileValue == Fast ? 10 : // 0.5 seconds
            ...
            profileValue == Bounce ? sizeof(_bounceProfile) - 1
                                   : // ~ 1.5 seconds
            duration * 2 +
                1; // Convert from deciseconds (100ms) to refresh cycles (50ms)
    s->stepNumber = 0;
    s->toPosition = value;
  ...
  // TODO: Could calculate step number from elapsed time, to allow for erratic
  // loop timing.
  void updatePosition(uint8_t pin) {
    ...
    if (s->stepNumber < s->numSteps) {
      ...
      if ((s->currentProfile & ~NoPowerOff) == Bounce) {
        ...
        s->currentPosition =
            map(profileValue, 0, 100, s->fromPosition, s->toPosition);
      } else {
        // All other profiles - calculate step by linear interpolation between
        // from and to positions.
        s->currentPosition =
            map(s->stepNumber, 0, s->numSteps, s->fromPosition, s->toPosition);
      }
  ...
  void _display() override {
    DIAG(F("Servo Configured on Vpins:%u-%u, slave pins:%d-%d %S"),
         (int)_firstVpin, (int)_firstVpin + _nPins - 1, (int)_firstSlavePin,
         (int)_firstSlavePin + _nPins - 1,
