...
DCC++ BASE STATION supports Sensor inputs that can be connected to any Arduino
Pin not in use by this program.  Sensors can be of any type (infrared, magentic,
mechanical...). The only requirement is that when "activated" the Sensor must
force the specified Arduino Pin LOW (i.e. to ground), and when not activated,
this Pin should remain HIGH (e.g. 5V), or be allowed to float HIGH if use of the
Arduino Pin's internal pull-up resistor is specified.
...
mechanical switches and transistors.  This avoids the need to create smoothing
circuitry for each sensor.  You may need to change the parameters through trial
and error for your specific sensors.
...
command:
  ...
  <S ID PIN PULLUP>:           creates a new sensor ID, with specified PIN and
PULLUP if sensor ID already exists, it is updated with specificed PIN and PULLUP
                               returns: <O> if successful and <X> if
unsuccessful (e.g. out of memory)

  <S ID>:                      deletes definition of sensor ID
                               returns: <O> if successful and <X> if
unsuccessful (e.g. ID does not exist)

  <S>:                         lists all defined sensors
                               returns: <Q ID PIN PULLUP> for each defined
...
definitions to EEPROM. If you later make edits/additions/deletions to the sensor
definitions, you must invoke the <E> command if you want those new definitions
updated in the EEPROM.  You can also clear everything stored in the EEPROM by
...
one state to another, one of the following serial messages are generated:
  ...
  <Q ID>     - for transition of Sensor ID from HIGH state to LOW state (i.e.
the sensor is triggered) <q ID>     - for transition of Sensor ID from LOW state
to HIGH state (i.e. the sensor is no longer triggered)
...
///////////////////////////////////////////////////////////////////////////////
// checks a number of defined sensors per entry and prints _changed_ sensor
// state to stream unless stream is NULL in which case only internal state is
// updated. Then advances to next sensor which will be checked at next
// invocation.  Each cycle of reading all sensors will be initiated no more
// frequently than the time set by 'cycleInterval' microseconds.
//
// The list of sensors is divided such that the first part of the list
// contains sensors that support change notification via callback, and the
// second part of the list contains sensors that require cyclic polling.  The
// start of the second part of the list is determined from by the
// 'firstPollSensor' pointer.
///////////////////////////////////////////////////////////////////////////////
...
void Sensor::checkAll() {
  ...
  while (readingSensor != NULL && !pause) {
    ...
    // Where the sensor is attached to a pin, read pin status.  For sources such
    // as LCN, which don't have an input pin to read, the LCN class calls
    // setState() to update inputState when a message is received.  The
    // IODevice::read() call returns 1 for active pins (0v) and 0 for inactive
    // (5v). Also, on HAL drivers that support change notifications, the driver
    // calls the notification callback routine when an input signal change is
    // detected, and this updates the inputState directly, so these inputs don't
    // need to be polled here.
    ...
    if (readingSensor->inputState == readingSensor->active) {
      ...
      CommandDistributor::broadcastSensor(readingSensor->data.snum,
                                          readingSensor->active);
    ...
    // Performance measurements taken during development indicate that, with 128
    // sensors configured on 8x 16-pin MCP23017 GPIO expanders with polling (no
    // change notification), all inputs can be read from the devices
    // within 1.4ms (400Mhz I2C bus speed), and a full cycle of checking 128
    // sensors for changes takes under a millisecond.
    sensorCount++;
    if (sensorCount >= 16)
      pause = true;
  }
...
// Updates the inputState field, which is subsequently scanned for changes in
// the checkAll method.  Ideally the <Q>/<q> message should be sent from here,
// instead of waiting for the checkAll method, but the output stream is not
// available at this point.
void Sensor::inputChangeCallback(VPIN vpin, int state) {
  Sensor *tt;
  // This bit is not ideal since it has, potentially, to look through the entire
  // list of sensors to find the one that has changed.  Ideally this should be
  // improved somehow.
...
Sensor *Sensor::create(int snum, VPIN pin, int pullUp) {
  ...
  remove(snum); // Unlink and free any existing sensor with the same id, before
  ...
  // for external infrared sensors --- each sensor must have its own 1K external
  // pull-up resistor
...
///////////////////////////////////////////////////////////////////////////////
// Object method to directly change the input state, for sensors such as LCN
// which are updated
...
bool Sensor::remove(int n) {
  ...
  for (tt = firstSensor; tt != NULL && tt->data.snum != n;
       pp = tt, tt = tt->nextSensor)
    ;
