...
MotorDriver::MotorDriver(int16_t power_pin, byte signal_pin, byte signal_pin2,
                         int16_t brake_pin, byte current_pin,
                         float sense_factor, unsigned int trip_milliamps,
                         int16_t fault_pin) {
  ...
  if (brake_pin != UNUSED_PIN) {
    ...
    // if brake is used for railcom  cutout we need to do PORTX register trick
    // here as well
  ...
  // This conversion performed at compile time so the remainder of the code
  // never needs float calculations or libraray code.
  ...
  if (currentPin == UNUSED_PIN)
    ...
    DIAG(F("Pin %d Max %dmA (%d)"), currentPin, raw2mA(rawCurrentTripValue),
         rawCurrentTripValue);

    // self testing diagnostic for the non-float converters... may be removed
    // when happy
...
void MotorDriver::setPower(POWERMODE mode) {
  ...
  if (on) {
    // when switching a track On, we need to check the crrentOffset with the pin
    // OFF
...
bool MotorDriver::canMeasureCurrent() { return currentPin != UNUSED_PIN; }
/*
 * Return the current reading as pin reading 0 to max resolution (1024 or 4096).
 * If the fault pin is activated return a negative current to show active fault
 * pin. As there is no -0, cheat a little and return -1 in that case.
 *
...
#endif
  ADMUX = (1 << REFS0) |
          ((currentPin - A0) & mask); // select AVCC as reference and set MUX
...
uint16_t taurustones[28] = {165, 175, 196, 220, 247, 262, 294, 330, 349, 392,
                            440, 494, 523, 587, 659, 698, 494, 440, 392, 249,
                            330, 284, 262, 247, 220, 196, 175, 165};
#endif
#endif
void MotorDriver::setDCSignal(byte speedcode) {
  ...
  switch (brakePin) {
    ...
    // If we are on pin 9 or 10 we are on Timer1 and we can not touch Timer1 as
    // that is our DCC source. If we are on pin 5 or 6 we are on Timer 0 ad we
    // can not touch Timer0 as that is millis() etc. We are most likely not on
    // pin 3 or 11 as no known motor shield has that as brake.
#endif
#if defined(ARDUINO_AVR_MEGA) || defined(ARDUINO_AVR_MEGA2560)
  case 9:
  case 10:
    // Timer2 (is differnet)
    TCCR2A = (TCCR2A & B11111100) |
             B00000001; // set WGM1=0 and WGM0=1 phase correct PWM
    TCCR2B =
        (TCCR2B & B11110000) |
        B00000110; // set WGM2=0 ; set divisor on timer 2 to 1/256 for 122.55Hz
  ...
  case 8:
    // Timer4
    TCCR4A = (TCCR4A & B11111100) |
             B00000001; // set WGM0=1 and WGM1=0 for normal PWM 8-bit
    TCCR4B = (TCCR4B & B11100000) |
             B00000100; // set WGM2=0 and WGM3=0 for normal PWM 8 bit and div
                        // 1/256 for 122.55Hz
    break;
  case 46:
  case 45:
  case 44:
    // Timer5
    TCCR5A = (TCCR5A & B11111100) |
             B00000001; // set WGM0=1 and WGM1=0 for normal PWM 8-bit
    TCCR5B = (TCCR5B & B11100000) |
             B00000100; // set WGM2=0 and WGM3=0 for normal PWM 8 bit and div
                        // 1/256 for 122.55Hz
    break;
#endif
  ...
  byte tSpeed =
      speedcode & 0x7F; // DCC Speed with 0,1 stop and speed steps 2 to 127
...
#if defined(ARDUINO_ARCH_ESP32)
  {
    ...
    DCCTimer::DCCEXanalogWriteFrequency(
        brakePin, f); // set DC PWM frequency to 100Hz XXX May move to setup
  }
#endif
  if (tSpeed <= 1)
    brake = 255;
...
void MotorDriver::throttleInrush(bool on) {
  ...
  if (on) {
...
#if defined(ARDUINO_AVR_UNO)
      // Not worth doin something here as:
      // If we are on pin 9 or 10 we are on Timer1 and we can not touch Timer1
      // as that is our DCC source. If we are on pin 5 or 6 we are on Timer 0 ad
      // we can not touch Timer0 as that is millis() etc. We are most likely not
      // on pin 3 or 11 as no known motor shield has that as brake.
#endif
#if defined(ARDUINO_AVR_MEGA) || defined(ARDUINO_AVR_MEGA2560)
    case 9:
    case 10:
      // Timer2 (is different)
      TCCR2A = (TCCR2A & B11111100) |
               B00000011; // set WGM0=1 and WGM1=1 for fast PWM
      TCCR2B = (TCCR2B & B11110000) |
               B00000001; // set WGM2=0 and prescaler div=1 (max)
      DIAG(F("2 A=%x B=%x"), TCCR2A, TCCR2B);
      break;
    case 6:
    case 7:
    case 8:
      // Timer4
      TCCR4A = (TCCR4A & B11111100) |
               B00000001; // set WGM0=1 and WGM1=0 for fast PWM 8-bit
      TCCR4B =
          (TCCR4B & B11100000) |
          B00001001; // set WGM2=1 and WGM3=0 for fast PWM 8 bit and div=1 (max)
      break;
    case 46:
    case 45:
    case 44:
...
unsigned int MotorDriver::raw2mA(int raw) {
  // DIAG(F("%d = %d * %d / %d"), (int32_t)raw * senseFactorInternal /
  // senseScale, raw, senseFactorInternal, senseScale);
  return (int32_t)raw * senseFactorInternal / senseScale;
}
unsigned int MotorDriver::mA2raw(unsigned int mA) {
  // DIAG(F("%d = %d * %d / %d"), (int32_t)mA * senseScale /
  // senseFactorInternal, mA, senseScale, senseFactorInternal);
  return (int32_t)mA * senseScale / senseFactorInternal;
}

void MotorDriver::getFastPin(const FSH *type, int pin, bool input,
                             FASTPIN &result) {
  ...
  // DIAG(F(" port=0x%x, inoutpin=0x%x, isinput=%d, mask=0x%x"),port,
  // result.inout,input,result.maskHIGH);
}

///////////////////////////////////////////////////////////////////////////////////////////
// checkPowerOverload(useProgLimit, trackno)
// bool useProgLimit: Trackmanager knows if this track is in prog mode or in
// main mode byte trackno: trackmanager knows it's number (could be skipped?)
//
...
void MotorDriver::checkPowerOverload(bool useProgLimit, byte trackno) {
  ...
  case POWERMODE::ALERT: {
    // set local flags that handle how much is output to diag (do not output
    // duplicates)
    ...
    if (checkFault()) {
      throttleInrush(true);
      lastBadSample = now;
      unsigned long timeout = checkCurrent(useProgLimit)
                                  ? POWER_SAMPLE_IGNORE_FAULT_HIGH
                                  : POWER_SAMPLE_IGNORE_FAULT_LOW;
      if (mslpc < timeout) {
        if (powerModeChange)
          DIAG(F("TRACK %c FAULT PIN (%M ignore)"), trackno + 'A', timeout);
        break;
      }
      DIAG(F("TRACK %c FAULT PIN detected after %4M. Pause %4M)"),
           trackno + 'A', mslpc, power_sample_overload_wait);
    ...
    if (checkCurrent(useProgLimit)) {
      ...
      if (mslpc < POWER_SAMPLE_IGNORE_CURRENT) {
        if (powerModeChange) {
          unsigned int mA = raw2mA(lastCurrent);
          DIAG(F("TRACK %c CURRENT (%M ignore) %dmA"), trackno + 'A',
               POWER_SAMPLE_IGNORE_CURRENT, mA);
        }
        break;
      }
      unsigned int mA = raw2mA(lastCurrent);
      unsigned int maxmA = raw2mA(tripValue);
      DIAG(F("TRACK %c POWER OVERLOAD %4dmA (max %4dmA) detected after %4M. "
             "Pause %4M"),
    ...
    if (goodtime > POWER_SAMPLE_ALERT_GOOD) {
      if (true || notFromOverload) { // we did a RESTORE message XXX
        unsigned int mA = raw2mA(lastCurrent);
        DIAG(F("TRACK %c NORMAL (after %M/%M) %dmA"), trackno + 'A', goodtime,
             mslpc, mA);
      }
      throttleInrush(false);
  ...
  case POWERMODE::OVERLOAD: {
    ...
    unsigned long mslpc =
        (commonFaultPin ? (micros() - globalOverloadStart)
                        : microsSinceLastPowerChange(POWERMODE::OVERLOAD));
