...
void DCCTimer::clearPWM() { TCCR1A = 0; }
...
#endif
        ADMUX =
            (1 << REFS0) | (id & 0x07); // select AVCC as reference and set MUX
...
void ADCee::begin() {
  noInterrupts();
  // ADCSRA = (ADCSRA & 0b11111000) | 0b00000100;   // speed up analogRead
  // sample time Set up ADC for free running mode
  ADMUX = (1 << REFS0); // select AVCC as reference. We set MUX later
  ADCSRA =
      (1 << ADEN) | (1 << ADPS2); // ADPS2 means divisor 32 and 16Mhz/32=500kHz.
  // bitSet(ADCSRA, ADSC); //do not start the ADC yet. Done when we have set the
  // MUX
  interrupts();
}
#endif
