...
class Sensor {
  // The sensor list is a linked list where each sensor's 'nextSensor' field
  // points to the next.
...
public:
...
#endif
  // readingSensor points to the next sensor to be polled, or null if the poll
  // cycle is completed for the period.
  ...
  static unsigned long
      lastReadCycle; // value of micros at start of last read cycle
  static const unsigned int cycleInterval =
      10000; // min time between consecutive reads of each sensor in microsecs.
             // should not be less than device scan cycle time.
  static const unsigned int minReadCount =
      1; // number of additional scans before acting on change
         // E.g. 1 means that a change is ignored for one scan and actioned on
         // the next. Max value is 63
