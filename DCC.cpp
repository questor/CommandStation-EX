...
// nor of the waveform issues such as preambles, start bits checksums or
// cutouts.
//
...
void DCC::setThrottle(uint16_t cab, uint8_t tSpeed, bool tDirection) {
  ...
  TrackManager::setDCSignal(
      cab, speedCode); // in case this is a dcc track on this addr
...
void DCC::setThrottle2(uint16_t cab, byte speedCode) {
  ...
  if (cab > HIGHEST_SHORT_ADDR)
    b[nB++] =
        highByte(cab) | 0xC0; // convert train number into a two-byte address
  b[nB++] = lowByte(cab);
  ...
  if (globalSpeedsteps <= 28) {
    ...
    if (speed128 == 0 || speed128 == 1) { // stop or emergency stop
      ...
      speed28 = (speed128 * 10 + 36) / 46; // convert 2-127 to 1-28
                                           /*
                                                 if (globalSpeedsteps <= 14)                   // Don't want to do 14
                                              steps, to get F0 there is ugly                                      code28 = (speed28+3)/2 | (Value of F0);
                                              // convert 1-28 to DCC 14 step speed code                                      else
                                           */
      code28 = (speed28 + 3) / 2 |
               ((speed28 & 1)
                    ? 0
                    : 0b00010000); // convert 1-28 to DCC 28 step speed code
    }
...
bool DCC::setFn(int cab, int16_t functionNumber, bool on) {
  ...
  if (functionNumber > 28) {
    ...
    if (functionNumber <= 127) {
      ...
      b[nB++] = (functionNumber & 0x7F) |
                (on ? 0x80 : 0);     // low order bits and state flag
...
// standard was made.
  ...
  // signifcant bits of accessory address second byte is of the form 1AAACPPG,
  // where C is 1 for on, PP the ports 0 to 3 and G the gate (coil).
  b[0] = address % 64 + 128;
  b[1] = ((((address / 64) % 8) << 4) + (port % 4 << 1) + gate % 2) ^ 0xF8;
  if (onoff != 0) {
    DCCWaveform::mainTrack.schedulePacket(b, 2,
                                          3); // Repeat on packet three times
  ...
  if (onoff != 1) {
    b[1] &= ~0x08; // set C to 0
    DCCWaveform::mainTrack.schedulePacket(b, 2,
                                          3); // Repeat off packet three times
  }
}
...
void DCC::writeCVByteMain(int cab, int cv, byte bValue) {
  ...
  b[nB++] =
      cv1(WRITE_BYTE_MAIN,
          cv); // any CV>1023 will become modulus(1024) due to bit-mask of 0x03
  b[nB++] = cv2(cv);
  b[nB++] = bValue;
...
FSH *DCC::getMotorShieldName() { return shieldName; }
...
const ackOp FLASH VERIFY_BYTE_PROG[] = {
    ...
    // There is no need for one validation as entire byte is validated at the
    // end
    V0, WACK, MERGE, // read and merge first tested bit (7)
    ITSKIP,          // do small excursion if there was no ack
    SETBIT, (ackOp)7, V1, WACK,
    NAKFAIL,          // test if there is an ack on the inverse of this bit (7)
    ...
    V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0,
    WACK, MERGE, VB, WACK,
...
const ackOp FLASH READ_CV_PROG[] = {
    ...
    V0, WACK, MERGE,             // read and merge third  tested bit (5) ...
    V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0,
    WACK, MERGE, VB, WACK, ITCB, // verify merged byte and return it if acked ok
    CALLFAIL};                   // verification failed

const ackOp FLASH LOCO_ID_PROG[] = {
    BASELINE, SETCV, (ackOp)19, // CV 19 is consist setting
    SETBYTE, (ackOp)0, VB, WACK,
    ITSKIP, // ignore consist if cv19 is zero (no consist)
    SETBYTE, (ackOp)128, VB, WACK,
    ITSKIP,     // ignore consist if cv19 is 128 (no consist, direction bit set)
    STARTMERGE, // Setup to read cv 19
    V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0,
    WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, VB, WACK,
    ITCB7, // return 7 bits only, No_ACK means CV19 not supported so ignore it

    SKIPTARGET, // continue here if CV 19 is zero or fails all validation
    SETCV, (ackOp)29, SETBIT, (ackOp)5, V0, WACK,
    ITSKIP, // Skip to SKIPTARGET if bit 5 of CV29 is zero

    // Long locoid
    SETCV, (ackOp)17,            // CV 17 is part of locoid
    STARTMERGE, V0, WACK, MERGE, // read and merge bit 1 etc
    V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0,
    WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, VB, WACK,
    NAKFAIL,     // verify merged byte and return -1 it if not acked ok
    ...
    // ITSKIP Skips to here if CV 29 bit 5 was zero. so read CV 1 and return
    // that
    SKIPTARGET, SETCV, (ackOp)1, STARTMERGE, SETBIT,
    (ackOp)6, // skip over first bit as we know its a zero
    V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, V0,
    WACK, MERGE, V0, WACK, MERGE, V0, WACK, MERGE, VB, WACK,
    ITCB, // verify merged byte and callback
    CALLFAIL};

const ackOp FLASH SHORT_LOCO_ID_PROG[] = {
    BASELINE, SETCV, (ackOp)19, SETBYTE, (ackOp)0, WB,
    WACK, // ignore dedcoder without cv19 support
    // Turn off long address flag
    SETCV, (ackOp)29, SETBIT, (ackOp)5, W0, WACK, V0, WACK, NAKFAIL, SETCV,
    (ackOp)1,
...
const ackOp FLASH LONG_LOCO_ID_PROG[] = {
    BASELINE,
    // Clear consist CV 19
    SETCV, (ackOp)19, SETBYTE, (ackOp)0, WB,
    WACK, // ignore decoder without cv19 support
...
void DCC::writeCVBit(int16_t cv, byte bitNum, bool bitValue,
                     ACK_CALLBACK callback) {
  if (bitNum >= 8)
    callback(-1);
  else
    DCCACK::Setup(cv, bitNum, bitValue ? WRITE_BIT1_PROG : WRITE_BIT0_PROG,
                  callback);
}
...
void DCC::verifyCVBit(int16_t cv, byte bitNum, bool bitValue,
                      ACK_CALLBACK callback) {
  if (bitNum >= 8)
    callback(-1);
  else
    DCCACK::Setup(cv, bitNum, bitValue ? VERIFY_BIT1_PROG : VERIFY_BIT0_PROG,
                  callback);
}
...
void DCC::issueReminders() {
  // if the main track transmitter still has a pending packet, skip this time
  // around.
...
bool DCC::issueReminder(int reg) {
  ...
  case 1: // remind function group 1 (F0-F4)
    if (flags & FN_GROUP_1)
      setFunctionInternal(loco, 0,
                          128 | ((functions >> 1) & 0x0F) |
                              ((functions & 0x01) << 4)); // 100D DDDD
    break;
  case 2: // remind function group 2 F5-F8
    if (flags & FN_GROUP_2)
      setFunctionInternal(loco, 0,
                          176 | ((functions >> 5) & 0x0F)); // 1011 DDDD
    break;
  case 3: // remind function group 3 F9-F12
    if (flags & FN_GROUP_3)
      setFunctionInternal(loco, 0,
                          160 | ((functions >> 9) & 0x0F)); // 1010 DDDD
    break;
