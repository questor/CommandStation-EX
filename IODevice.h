...
// Define symbol IO_NO_HAL to reduce FLASH footprint when HAL features not
// required The HAL is disabled by default on Nano and Uno platforms, because of
// limited flash space.
...
// Limit VPIN number to max 32767.  Above this number, printing often gives
// negative values. This should be enough for 99% of users.
...
public:
  ...
  static bool hasCallback() { return first != NULL; }
...
public:
  ...
  static bool configure(VPIN vpin, ConfigTypeEnum configType, int paramCount,
                        int params[]);
  ...
  inline static bool configureServo(VPIN vpin, uint16_t activePosition,
                                    uint16_t inactivePosition,
                                    uint8_t profile = 0, uint16_t duration = 0,
                                    uint8_t initialState = 0) {
    int params[] = {(int)activePosition, (int)inactivePosition, profile,
                    (int)duration, initialState};
  ...
  // write invokes the IODevice instance's _writeAnalogue method (not applicable
  // for digital outputs)
  static void writeAnalogue(VPIN vpin, int value, uint8_t profile = 0,
                            uint16_t duration = 0);

  // isBusy returns true if the device is currently in an animation of some
  // sort, e.g. is changing
  //  the output over a period of time.
  static bool isBusy(VPIN vpin);

  // check whether the pin supports notification.  If so, then regular _read
  ...
  // Enable shared interrupt on specified pin for GPIO extender modules.  The
  // extender module should pull down this pin when requesting a scan.  The pin
  // may be shared by multiple modules. Without the shared interrupt, input
  // states are scanned periodically to detect changes on GPIO extender pins. If
  // a shared interrupt pin is configured, then input states are scanned only
  // when the shared interrupt pin is pulled low.  The external GPIO module
  // releases the pin once the GPIO port concerned has been read.
  ...
  // Method used by IODevice filters to locate slave pins that may be overlayed
  // by their own pin range.
  ...
  // Method to write an 'analogue' value (optionally implemented within device
  // class)
  virtual void _writeAnalogue(VPIN vpin, int value, uint8_t param1 = 0,
                              uint16_t param2 = 0) {
    (void)vpin;
    (void)value;
    (void)param1;
  ...
  // Method to read digital pin state (optionally implemented within device
  // class)
  ...
  // Method to read analogue pin state (optionally implemented within device
  // class)
...
protected:
  ...
  // Method to perform initialisation of the device (optionally implemented
  // within device class)
  virtual void _begin() {}
  ...
  virtual bool _configure(VPIN vpin, ConfigTypeEnum configType, int paramCount,
                          int params[]) {
    (void)vpin;
    (void)configType;
    (void)paramCount;
  ...
  // Method to perform updates on an ongoing basis (optionally implemented
  // within device class)
  virtual void _loop(unsigned long currentMicros) {
    delayUntil(currentMicros + 0x7fffffff); // Largest time in the future!
                                            // Effectively disable _loop calls.
  };

  // Method for displaying info on DIAG output (optionally implemented within
  // device class)
  ...
  // Pin number of interrupt pin for GPIO extender devices.  The extender module
  // will pull this
...
private:
  ...
  int _read(VPIN vpin)
      override; // returns the digital state or busy status of the device
  ...
  struct ServoData {
    ...
    uint16_t
        numSteps; // Number of steps in animation, or 0 if none in progress.
  ...
  static const uint8_t _catchupSteps =
      5; // number of steps to wait before switching servo off
...
public:
  static void create(VPIN firstVpin, int nPins, int DCCAddress,
                     int DCCSubaddress);

private:
  // Constructor
  DCCAccessoryDecoder(VPIN firstVpin, int nPins, int DCCAddress,
                      int DCCSubaddress);
...
private:
  ...
  void _loop(unsigned long currentMicros) override;
  int _read(VPIN vpin) override;
  void _writeAnalogue(VPIN vpin, int value, uint8_t activity,
                      uint16_t duration) override;
  void _display() override;
...
protected:
  ...
  void _display() override { DIAG(F("UserAddin run every %dms"), _delay); }
};
...
protected:
  ...
  void _display() override {
    DIAG(F("FLAGS configured on VPINs %u-%u"), _firstVpin,
         _firstVpin + _nPins - 1);
  }
};
