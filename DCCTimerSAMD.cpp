...
void DCCTimer::begin(INTERRUPT_CALLBACK callback) {
  ...
  // Assume we're using TCC0... as we're bit-bashing the DCC waveform output
  // pins anyway for "normal accuracy" DCC waveform generation. For high
  // accuracy we're going to need to a good deal more. The TCC waveform output
  // pins are mux'd on the SAMD, and output pins for each TCC are only available
  // on certain pins
...
int ADCee::init(uint8_t pin) {
  ...
  ADC->INPUTCTRL.bit.MUXPOS =
      g_APinDescription[pin]
          .ulADCChannelNumber; // Selection for the positive ADC input
...
void ADCee::begin() {
  ...
  // is 436uS, and we need sub-58uS. This code sets it to a read speed of around
  // 5-6uS, and enables 12-bit mode Reconfigure ADC
