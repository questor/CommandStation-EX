...
public:
  ...
  PCA9555(VPIN vpin, int nPins, uint8_t I2CAddress, int interruptPin = -1)
      : GPIOBase<uint16_t>((FSH *)F("PCA9555"), vpin, nPins, I2CAddress,
                           interruptPin) {
...
private:
  void _writeGpioPort() override {
    I2CManager.write(_I2CAddress, 3, REG_OUTPUT_P0, _portOutputState,
                     _portOutputState >> 8);
  }
  ...
  void _writePortModes() override {
    // Write 0 to REG_CONF_P0 & REG_CONF_P1 for in-use pins that are outputs, 1
    // for others. PCA9555 & TCA9555, Interrupt is always enabled for raising
    // and falling edge
  ...
  void _readGpioPort(bool immediate) override {
    if (immediate) {
      ...
      /* PCA9555 Int bug fix, from PCA9555 datasheet: "must change command byte
       * to something besides 00h after a Read operation to the PCA9555 device
       * or before reading from another device" Recommended solution, read from
       * REG_OUTPUT_P0, then do nothing with the received data Issue not seen
       * during testing, uncomment if needed
       */
