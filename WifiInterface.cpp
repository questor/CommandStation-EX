...
// This code is NOT compiled on a unoWifiRev2 processor which uses a different
// architecture
...
bool WifiInterface::setup(long serial_link_speed, const FSH *wifiESSID,
...
// Currently only the Arduino Mega 2560 has usable Serial2 (Nucleo-64 boards use
// Serial 2 for console!)
...
wifiSerialState WifiInterface::setup(Stream &setupStream, const FSH *SSid,
                                     const FSH *password, const FSH *hostname,
                                     int port, byte channel, bool forceAP) {
...
wifiSerialState WifiInterface::setup2(const FSH *SSid, const FSH *password,
  ...
  // If there is, just shortcut the setup and continue to read the data as
  // normal.
  ...
  StringFormatter::send(wifiStream,
  ...
  if (checkForOK(2000, F("AT version:"), true, false)) {
    ...
    if ((version[0] == '0') || (version[0] == '2' && version[2] == '0') ||
        (version[0] == '2' && version[2] == '2' && version[4] == '0' &&
         version[6] == '0' && version[7] == '-' && version[8] == 'd' &&
...
#else
  // Older ES versions have AT+CWJAP, newer ones have AT+CWJAP_CUR and
  // AT+CWHOSTNAME
  ...
  if (!(checkForOK(2000, true))) {
    ...
    while (wifiStream->available())
      StringFormatter::printEscape(
          wifiStream->read()); /// THIS IS A DIAG IN DISGUISE
  }

  StringFormatter::send(
      wifiStream, F("AT+CWMODE%s=1\r\n"),
  ...
  if (STRNCMP_P(yourNetwork, (const char *)SSid, 13) == 0 ||
      STRNCMP_P("", (const char *)SSid, 13) == 0) {
    ...
    do {
      if (!forceAP) {
        if (STRNCMP_P(yourNetwork, (const char *)password, 13) == 0) {
          // unconfigured
          StringFormatter::send(
              wifiStream, F("AT+CWSAP%s=\"DCCEX_%s\",\"PASS_%s\",%d,4\r\n"),
              oldCmd ? "" : "_CUR", macTail, macTail, channel);
        } else {
          // password configured by user
          StringFormatter::send(
              wifiStream, F("AT+CWSAP%s=\"DCCEX_%s\",\"%S\",%d,4\r\n"),
              oldCmd ? "" : "_CUR", macTail, password, channel);
        }
      } else {
        StringFormatter::send(wifiStream,
                              F("AT+CWSAP%s=\"%S\",\"%S\",%d,4\r\n"),
                              oldCmd ? "" : "_CUR", SSid, password, channel);
      }
    } while (!checkForOK(WIFI_CONNECT_TIMEOUT, true) &&
             i++ < 2); // do twice if necessary but ignore failure as AP mode
                       // may still be ok
    if (i >= 2)
      DIAG(F(
          "Warning: Setting AP SSID and password failed")); // but issue warning
  ...
  // Copy the IP address
  {
    ...
    ipString[MAX_IP_LENGTH] =
        '\0'; // protection against missing " character on end.
    ...
    LCD(4, F("%s"), ipString); // There is not enough room on some LCDs to put a
                               // title to this
  }
...
// This function is used to allow users to enter <+ commands> through the
// DCCEXParser
// <+command>  sends AT+command to the ES and returns to the caller.
// Once the user has made whatever changes to the AT commands, a <+X> command
// can be used to force on the connectd flag so that the loop will start picking
// up wifi traffic. If the settings are corrupted <+RST> will clear this and
// then you must restart the arduino.

// Using the <+> command with no command string causes the code to enter an echo
// loop so that all input is directed to the ES and all ES output written to the
// USB Serial. The sequence "!!!" returns the Arduino to the normal loop mode
...
void WifiInterface::ATCommand(HardwareSerial *stream, const byte *command) {
  ...
  if (*command == '\0') { // User gave <+> command
    ...
    while (stream->available())
      stream->read(); // Drain serial input first
    bool startOfLine = true;
    while (true) {
      while (wifiStream->available())
        stream->write(wifiStream->read());
      if (stream->available()) {
        int cx = stream->read();
...
bool WifiInterface::checkForOK(const unsigned int timeout, bool echo,
                               bool escapeEcho) {
  return checkForOK(timeout, F("\r\nOK\r\n"), echo, escapeEcho);
}

bool WifiInterface::checkForOK(const unsigned int timeout, const FSH *waitfor,
                               bool echo, bool escapeEcho) {
