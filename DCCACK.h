...
enum ackOp : byte { // Program opcodes for the ack Manager
  ...
  MERGE,       // Merge previous wack response with byte value and decrement bit
               // number (use for readimng CV bytes)
...
enum CALLBACK_STATE : byte {
  ...
  AFTER_READ,  // Start callback sequence after something was read from the
               // decoder
  AFTER_WRITE, // Start callback sequence after something was written to the
               // decoder
...
public:
  ...
  static void
  checkAck(byte sentResetsSincePacket); // Interrupt time ack checker
  static inline void setAckLimit(int mA) { ackLimitmA = mA; }
  ...
  static void Setup(int cv, byte byteValueOrBitnum, ackOp const program[],
...
private:
  ...
  static const int PROG_REPEATS = 8; // repeats of programming commands (some
                                     // decoders need at least 8 to be reliable)
