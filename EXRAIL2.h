...
// The following are the operation codes (or instructions) for a kind of virtual
// machine. Each instruction is normally 3 bytes long with an operation code
// followed by a parameter. In cases where more than one parameter is required,
// the first parameter is followed by one or more OPCODE_PAD instructions with
// the subsequent parameters. This wastes a byte but makes searching easier as a
// parameter can never be confused with an opcode.
//
enum OPCODE : byte {
  OPCODE_THROW,
  OPCODE_CLOSE,
...
private:
  static void ComandFilter(Print *stream, byte &opcode, byte &paramCount,
                           int16_t p[]);
  ...
  static LookList *LookListLoader(OPCODE op1, OPCODE op2 = OPCODE_ENDEXRAIL,
                                  OPCODE op3 = OPCODE_ENDEXRAIL);
  ...
  unsigned long
      delayStart; // Used by opcodes that must be recalled before completing
