 ...
 * Credit is due to  Valerie Valley RR
 * https://sites.google.com/site/valerievalleyrr/ for showing how it could be
 * done, but this code is very different to the original implementation as it is
 * designed to run on the Arduino and not the ESP and is also calling directly
 * into the DCCEX Api rather than  simulating JMRI text commands. Refer JMRI
 * WiFi Throttle Communications Protocol
 * https://www.jmri.org/help/en/package/jmri/jmrit/withrottle/Protocol.shtml
 *
 *
 * PROTOTYPE NOTES:
 *  There will be one WiThrottle instance created for each WiThrottle client
 * detected by the WifiInterface. Some shortcuts have been taken and there are
 * some things that are yet to be included: e.g. Full response to adding a loco.
 ...
 *    - I suggest that at the end of parse(), then anything that has changed and
 * is of interest could be notified then.  (e.g loco speeds, directions or
 * functions, turnout states.
 *
 *  WiThrottle.h sets the max locos per client at 10, this is ok to increase but
 * requires just an extra 3 bytes per loco per client.
 */
...
#define LOOPLOCOS(THROTTLECHAR, CAB)                                           \
  for (int loco = 0; loco < MAX_MY_LOCO; loco++)                               \
    if ((myLocos[loco].throttle == THROTTLECHAR || '*' == THROTTLECHAR) &&     \
        (CAB < 0 || myLocos[loco].cab == CAB))
...
// One instance of WiThrottle per connected client, so we know what the locos
// are
  ...
  while (cmd[0]) {
    ...
    case 'P':
      if (cmd[1] == 'P' && cmd[2] == 'A') { // PPA power mode
        TrackManager::setMainPower(cmd[3] == '1' ? POWERMODE::ON
                                                 : POWERMODE::OFF);
        /* TODO
                if (MotorDriver::commonFaultPin) // commonFaultPin prevents
           individual track handling
        ...
        */
      ...
      else if (cmd[1] == 'R' && cmd[2] == 'A' &&
               cmd[3] == '2') { // Route activate
        ...
        switch (cmd[3]) {
          // T and C according to RCN-213 where 0 is Stop, Red, Thrown,
          // Diverging.
        case 'T':
      ...
      break;
    case 'N': // Heartbeat (2), only send if connection completed by 'HU'
              // message
      StringFormatter::send(stream, F("*%d\n"),
                            heartrateSent
                                ? HEARTBEAT_SECONDS
                                : HEARTBEAT_PRELOAD); // return timeout value
      break;
    ...
    case 'H': // send initial connection info after receiving "HU" message
      ...
      break;
    case 'Q':              //
      LOOPLOCOS('*', -1) { // tell client to drop any locos still assigned to
                           // this WiThrottle
        if (myLocos[loco].throttle != '\0') {
          StringFormatter::send(stream, F("M%c-%c%d<;>\n"),
                                myLocos[loco].throttle, LorS(myLocos[loco].cab),
                                myLocos[loco].cab);
        }
      }
      if (Diag::WITHROTTLE)
...
void WiThrottle::multithrottle(RingStream *stream, byte *cmd) {
  ...
  case '+': // add loco request
    if (cmd[3] == '*') {
      // M+* means get loco from prog track, then join tracks ready to drive
      // away Stash the things the callback will need later
      stashStream = stream;
      ...
      return; // return nothing in stream as response is sent later in the
              // callback
    }
    ...
    // use first empty "slot" on this client's list, will be added to DCC
    // registration list
    for (int loco = 0; loco < MAX_MY_LOCO; loco++) {
      if (myLocos[loco].throttle == '\0') {
        ...
        myLocos[loco].broadcastPending =
            true; // means speed/dir will be sent later
        mostRecentCab = locoid;
        StringFormatter::send(stream, F("M%c+%c%d<;>\n"), throttleChar, cmd[3],
                              locoid); // tell client to add loco
        sendFunctions(stream, loco);
        // speed and direction will be published at next broadcast cycle
        StringFormatter::send(stream, F("M%cA%c%d<;>s1\n"), throttleChar,
                              cmd[3], locoid); // default speed step 128
        return;
      }
    }
    StringFormatter::send(stream,
                          F("HMMax locos (%d) exceeded, %d not added!\n"),
                          MAX_MY_LOCO, locoid);
    break;
  case '-': // remove loco(s) from this client (leave in DCC registration)
    LOOPLOCOS(throttleChar, locoid) {
      myLocos[loco].throttle = '\0';
      StringFormatter::send(stream, F("M%c-%c%d<;>\n"), throttleChar,
                            LorS(myLocos[loco].cab), myLocos[loco].cab);
    }
...
void WiThrottle::locoAction(RingStream *stream, byte *aval, char throttleChar,
                            int cab) {
  // Note cab=-1 for all cabs in the consist called throttleChar.
  //    DIAG(F("Loco Action aval=%c%c throttleChar=%c, cab=%d"),
  //    aval[0],aval[1],throttleChar, cab);
  (void)stream;
  switch (aval[0]) {
  case 'V': // Vspeed
  {
    ...
    LOOPLOCOS(throttleChar, cab) {
      ...
      DCC::setThrottle(myLocos[loco].cab, WiTToDCCSpeed(witSpeed),
                       DCC::getThrottleDirection(myLocos[loco].cab));
  ...
  case 'X':
    ...
    LOOPLOCOS(throttleChar, cab) {
      DCC::setThrottle(myLocos[loco].cab, 1,
                       DCC::getThrottleDirection(myLocos[loco].cab));
      // setThrottle will cause a broadcast so notification will be sent
    }
    break;
  ...
  case 'Q': // Quit, set speed to 0
    LOOPLOCOS(throttleChar, cab) {
      mostRecentCab = myLocos[loco].cab;
      DCC::setThrottle(myLocos[loco].cab, 0,
                       DCC::getThrottleDirection(myLocos[loco].cab));
      // setThrottle will cause a broadcast so notification will be sent
    }
    break;
...
void WiThrottle::checkHeartbeat(RingStream *stream) {
  // if eStop time passed... eStop any locos still assigned to this client and
  // then drop the connection
  if (heartBeatEnable && (millis() - heartBeat > ESTOP_SECONDS * 1000)) {
    if (Diag::WITHROTTLE)
      DIAG(F("%l WiThrottle(%d) eStop(%ds) timeout, drop connection"), millis(),
           clientid, ESTOP_SECONDS);
    LOOPLOCOS('*', -1) {
      if (myLocos[loco].throttle != '\0') {
        if (Diag::WITHROTTLE)
        ...
        DCC::setThrottle(
            myLocos[loco].cab, 1,
            DCC::getThrottleDirection(myLocos[loco].cab)); // speed 1 is eStop
        heartBeat = millis(); // We have just stopped everyting, we don't need
                              // to do that again at next loop.
      }
    }
  ...
  // send any outstanding speed/direction/function changes for this clients
  // locos Changes may have been caused by this client, or another
  // non-Withrottle or Exrail
  ...
  LOOPLOCOS('*', -1) {
    if (myLocos[loco].throttle != '\0' && myLocos[loco].broadcastPending) {
      ...
      StringFormatter::send(stream, F("M%cA%c%d<;>V%d\n"), throttle, lors, cab,
                            DCCToWiTSpeed(DCC::getThrottleSpeed(cab)));
...
void WiThrottle::markForBroadcast2(int cab) {
  LOOPLOCOS('*', cab) { myLocos[loco].broadcastPending = true; }
}
...
void WiThrottle::getLocoCallback(int16_t locoid) {
  ...
  if (locoid & LONG_ADDR_MARKER) { // maker bit indicates long addr
    locoid =
        locoid ^ LONG_ADDR_MARKER; // remove marker bit to get real long addr
    if (locoid <= HIGHEST_SHORT_ADDR) { // out of range for long addr
      StringFormatter::send(stashStream,
                            F("HMLong addr %d <= %d unsupported\n"), locoid,
                            HIGHEST_SHORT_ADDR);
...
void WiThrottle::sendIntro(Print *stream) {
  ...
  StringFormatter::send(stream, F("HtDCC-EX v%S, %S, %S, %S\n"), F(VERSION),
                        F(ARDUINO_TYPE), DCC::getMotorShieldName(),
                        F(GITHUB_SHA));
  StringFormatter::send(
      stream, F("PTT]\\[Turnouts}|{Turnout]\\[THROW}|{2]\\[CLOSE}|{4\n"));
  StringFormatter::send(stream, F("PPA%x\n"),
                        TrackManager::getMainPower() == POWERMODE::ON);
  // set heartbeat to 2 seconds because we need to sync the metadata (1 second
  // is too short!)
  StringFormatter::send(stream, F("*%d\nHMConnecting..\n"), HEARTBEAT_PRELOAD);
}
  ...
  for (int16_t r = 0;; r++) {
    ...
    if (cabid > 0)
      StringFormatter::send(stream, F("]\\[%S}|{%d}|{%c"),
                            RMFT2::getRosterName(cabid), cabid,
                            cabid < 128 ? 'S' : 'L');
  }
  StringFormatter::send(stream, F("\n"));
#else
...
#ifdef EXRAIL_ACTIVE
  StringFormatter::send(
      stream, F("PRT]\\[Routes}|{Route]\\[Set}|{2]\\[Handoff}|{4\nPRL"));
...
void WiThrottle::sendFunctions(Print *stream, byte loco) {
  ...
  if (functionNames == NULL) {
    ...
    StringFormatter::send(stream, F("M%cL%c%d<;>]\\["), myLocos[loco].throttle,
                          LorS(locoid), locoid);
    fkeys = 0;
    bool firstchar = true;
    for (int fx = 0;; fx++) {
  ...
  for (int fKey = 0; fKey < fkeys; fKey++) {
    ...
    if (fstate >= 0)
      StringFormatter::send(stream, F("M%cA%c%d<;>F%d%d\n"),
                            myLocos[loco].throttle, LorS(locoid), locoid,
                            fstate, fKey);
  }
}
