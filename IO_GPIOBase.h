...
// GPIOBase is defined as a class template.  This allows it to be instantiated
// by subclasses with different types, according to the number of pins on the
// GPIO module. For example, GPIOBase<uint8_t> for 8 pins, GPIOBase<uint16_t>
// for 16 pins etc. A module with up to 64 pins can be handled in this way
// (uint64_t).

template <class T> class GPIOBase : public IODevice {

protected:
  // Constructor
  GPIOBase(FSH *deviceName, VPIN firstVpin, uint8_t nPins,
           I2CAddress i2cAddress, int interruptPin);
...
// Because class GPIOBase is a template, the implementation (below) must be
// contained within the same file as the class declaration (above).  Otherwise
// it won't compile!

// Constructor
template <class T>
GPIOBase<T>::GPIOBase(FSH *deviceName, VPIN firstVpin, uint8_t nPins,
                      I2CAddress i2cAddress, int interruptPin)
    : IODevice(firstVpin, nPins) {
  if (_nPins > (int)sizeof(T) * 8)
    _nPins = sizeof(T) *
             8; // Ensure nPins is consistent with the number of bits in T
...
template <class T> void GPIOBase<T>::_begin() {
  ...
  if (I2CManager.exists(_I2CAddress)) {
    ...
    DIAG(F("%S I2C:%s Device not detected"), _deviceName,
...
template <class T>
bool GPIOBase<T>::_configure(VPIN vpin, ConfigTypeEnum configType,
...
#ifdef DIAG_IO
  DIAG(F("%S I2C:%s Config Pin:%d Val:%d"), _deviceName, _I2CAddress.toString(),
       pin, pullup);
#endif
  uint16_t mask = 1 << pin;
  if (pullup)
...
template <class T> void GPIOBase<T>::_loop(unsigned long currentMicros) {
  ...
  if (_deviceState == DEVSTATE_SCANNING && !requestBlock.isBusy()) {
    ...
    if (status == I2C_STATUS_OK) {
      ...
      _deviceState = DEVSTATE_FAILED;
      DIAG(F("%S I2C:%s Error:%d %S"), _deviceName, _I2CAddress.toString(),
    ...
    if (differences && IONotifyCallback::hasCallback()) {
      ...
      for (int pin = 0; pin < _nPins; pin++) {
        if (differences & mask) {
          // Change detected.
          IONotifyCallback::invokeAll(_firstVpin + pin,
                                      (_portInputState & mask) == 0);
        }
        mask <<= 1;
      }
    }
...
#ifdef DIAG_IO
    if (differences)
      DIAG(F("%S I2C:%s PortStates:%x"), _deviceName, _I2CAddress.toString(),
           _portInputState);
#endif
  }

  // Check if interrupt configured.  If not, or if it is active (pulled down),
  // then
  //  initiate a scan.
...
template <class T> void GPIOBase<T>::_display() {
  DIAG(F("%S I2C:%s Configured on Vpins:%u-%u %S"), _deviceName,
       _I2CAddress.toString(), _firstVpin, _firstVpin + _nPins - 1,
       (_deviceState == DEVSTATE_FAILED) ? F("OFFLINE") : F(""));
}

template <class T> void GPIOBase<T>::_write(VPIN vpin, int value) {
  int pin = vpin - _firstVpin;
  T mask = 1 << pin;
#ifdef DIAG_IO
  DIAG(F("%S I2C:%s Write Pin:%d Val:%d"), _deviceName, _I2CAddress.toString(),
       pin, value);
#endif
...
template <class T> int GPIOBase<T>::_read(VPIN vpin) {
  int pin = vpin - _firstVpin;
  T mask = 1 << pin;
