 ...
 *
 * Alternative configuration, which hogs the processor until the measurement is
 * complete (old behaviour, more accurate but higher impact on other CS tasks):
...
public:
  enum Options {
    LOOP = 1, // Option HCSR04::LOOP reinstates old behaviour, i.e. complete
              // measurement in one loop entry.
  };

  // Static create function provides alternative way to create object
  static void create(VPIN vpin, int trigPin, int echoPin, uint16_t onThreshold,
                     uint16_t offThreshold, uint16_t options = 0) {
...
protected:
  ...
  HCSR04(VPIN vpin, int trigPin, int echoPin, uint16_t onThreshold,
         uint16_t offThreshold, uint16_t options) {
    _firstVpin = vpin;
    _nPins = 1;
    _trigPin = trigPin;
    _echoPin = echoPin;
  ...
  void _loop(unsigned long currentMicros) override {
    ...
    case DORMANT: // Issue pulse
      // If receive pin is still set on from previous call, do nothing till next
      // entry.
      ...
      // Echo pulse started, so wait for echo pin to reset, and measure length
      // of pulse
      _startTime = micros();
      _maxTime = factor * _offThreshold;
      _state = MEASURING;
      // If maximum measurement time is high, then skip until next loop entry
      // before starting to look for pulse end. This gives better accuracy at
      // shorter distance thresholds but without extending loop execution time
      // for longer thresholds.  If LOOP option is set on, then the entire
      // measurement will be done in one loop entry, i.e. the code will fall
    ...
    case MEASURING: // Check if echo pulse has finished
      do {
        waitTime = micros() - _startTime;
        if (!ArduinoPins::fastReadDigital(_echoPin)) {
          // Echo pulse completed; check if pulse length is below threshold and
          // if so set value.
          if (waitTime <= factor * _onThreshold) {
            // Measured time is within the onThreshold, so value is one.
            _value = 1;
            // If the new distance value is less than the current, use it
            // immediately. But if the new distance value is longer, then it may
            // be erroneously long (because of extended loop times delays), so
            // apply a delay to distance increases.
          ...
          if (waitTime > _maxTime) {
            ...
            // But don't change _value unless provisional value is zero for 10
        ...
        // If there's lots of time remaining before the expected completion
        // time, then exit and wait for next loop entry.  Otherwise, loop until
        // we finish. If option LOOP is set, then we loop until finished anyway.
  ...
  void _display() override {
    DIAG(F("HCSR04 Configured on VPIN:%u TrigPin:%d EchoPin:%d On:%dcm "
           "Off:%dcm"),
