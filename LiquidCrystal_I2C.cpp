...
bool LiquidCrystal_I2C::begin() {
  ...
  if (I2CManager.exists(_Addr)) {
    DIAG(F("%dx%d LCD configured on I2C:%s"), (int)lcdCols, (int)lcdRows,
         _Addr.toString());
...
bool LiquidCrystal_I2C::isBusy() { return rb.isBusy(); }
 ...
 *
 * This driver takes advantage of this by sending multiple data bytes in
 * succession within a single I2C transmission.  With a fast clock rate of
 * 400kHz, the time between successive updates of the PCF8574 outputs will be at
 * least 2.5us.  With the default clock rate of 100kHz the time between updates
 * will be at least 10us.
 *
 * The LCD controller HD44780, according to its datasheet, needs nominally 37us
 * (up to 50us) to execute a command (i.e. write to gdram, reposition, etc.).
 * Each command is sent in a separate I2C transmission here.  The time taken to
 * end a transmission and start another one is a stop bit, a start bit, 8
 * address bits, an ack, 8 data bits and another ack; this is at least 20 bits,
 * i.e. >50us at 400kHz and >200us at 100kHz. Therefore, we don't need
 * additional delay.
 *
 ...
 * as we present the data, then in the next byte we reset Enable without
 * changing the data.
 */
...
void LiquidCrystal_I2C::send(uint8_t value, uint8_t mode) {
  ...
  I2CManager.write(_Addr, outputBuffer, len,
                   &rb); // Write command asynchronously
}
