...
DCC++ BASE STATION supports optional OUTPUT control of any unused Arduino Pins
...
Definitions and state (ACTIVE/INACTIVE) for pins are retained in EEPROM and
...
command:
  ...
  <Z ID PIN IFLAG>:            creates a new output ID, with specified PIN and
IFLAG values. if output ID already exists, it is updated with specificed PIN and
IFLAG. note: output state will be immediately set to ACTIVE/INACTIVE and pin
will be set to HIGH/LOW according to IFLAG value specifcied (see below).
...
unsuccessful (e.g. ID does not exist)
  ...
  <Z>:                         lists all defined output pins
                               returns: <Y ID PIN IFLAG STATE> for each defined
output pin or <X> if no output pins defined
...
2 as follows:
          ...
          IFLAG, bit 1:   0 = state of pin restored on power-up to either ACTIVE
or INACTIVE depending on state before power-down; state of pin set to INACTIVE
when first created 1 = state of pin set on power-up, or when first created, to
either ACTIVE of INACTIVE depending on IFLAG, bit 2

          IFLAG, bit 2:   0 = state of pin set to INACTIVE uponm power-up or
when first created 1 = state of pin set to ACTIVE uponm power-up or when first
created

Once all outputs have been properly defined, use the <E> command to store their
definitions to EEPROM. If you later make edits/additions/deletions to the output
...
To change the state of outputs that have been defined use:
  ...
  <Z ID STATE>:                sets output ID to either ACTIVE or INACTIVE state
                               returns: <Y ID STATE>, or <X> if turnout ID does
not exist
...
output in EEPROM so that it is retained even without power.  A list of the
current states of each output in the form <Y ID STATE> is generated by this
sketch whenever the <s> status command is invoked.  This provides an efficient
way of initializing the state of any outputs being monitored or controlled by a
separate interface or GUI program.
...
///////////////////////////////////////////////////////////////////////////////
// Static function to print all output states to stream in the form "<Y id
// state>"
...
bool Output::remove(uint16_t n) {
  ...
  for (tt = firstOutput; tt != NULL && tt->data.id != n;
       pp = tt, tt = tt->nextOutput)
    ;
...
void Output::load() {
  ...
  for (uint16_t i = 0; i < EEStore::eeStore->data.nOutputs; i++) {
    ...
    // Create new object, set current state to default or to saved state from
    // eeprom.
    ...
    if (tt)
      tt->num =
          EEStore::pointer() +
          offsetof(OutputData, oStatus); // Save pointer to flags within EEPROM
...
Output *Output::create(uint16_t id, VPIN pin, int iFlag, int v) {
  ...
  if (tt == NULL)
    return tt;
  tt->num = 0; // make sure new object doesn't get written to EEPROM until
               // store() command
  tt->data.id = id;
  tt->data.pin = pin;
  tt->data.flags = iFlag;
  ...
  if (v == 1) {
    // sets status to 0 (INACTIVE) is bit 1 of iFlag=0, otherwise set to value
    // of bit 2 of iFlag
