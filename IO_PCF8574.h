...
private:
  PCF8574(VPIN firstVpin, uint8_t nPins, I2CAddress i2cAddress,
          int interruptPin = -1)
      : GPIOBase<uint8_t>((FSH *)F("PCF8574"), firstVpin, nPins, i2cAddress,
                          interruptPin) {
    requestBlock.setReadParams(_I2CAddress, inputBuffer, 1);
  }

  // The PCF8574 handles inputs by applying a weak pull-up when output is driven
  // to '1'. The pin state is driven '1' if the pin is an input, or if it is an
  // output set to 1. Unused pins are driven '0'.
  void _writeGpioPort() override {
    I2CManager.write(_I2CAddress, 1,
                     (_portOutputState | ~_portMode) & _portInUse);
  }
  ...
  // Set up device ports
  void _setupDevice() override { _writePortModes(); }
