...
private:
  PCF8575(VPIN firstVpin, uint8_t nPins, I2CAddress i2cAddress,
          int interruptPin = -1)
      : GPIOBase<uint16_t>((FSH *)F("PCF8575"), firstVpin, nPins, i2cAddress,
                           interruptPin) {
    requestBlock.setReadParams(_I2CAddress, inputBuffer, sizeof(inputBuffer));
  }

  // The PCF8575 handles inputs by applying a weak pull-up when output is driven
  // to '1'. The pin state is driven '1' if the pin is an input, or if it is an
  // output set to 1. Unused pins are driven '0'.
  ...
  // The PCF8575 handles inputs by applying a weak pull-up when output is driven
  // to '1'. Therefore, writing '1' in _writePortModes is enough to set the
  // module to input mode and enable pull-up.
  void _writePullups() override {}

  // The pin state is '1' if the pin is an input or if it is an output set to 1.
  // Zero otherwise.
  void _writePortModes() override { _writeGpioPort(); }

  // In immediate mode, _readGpioPort reads the device GPIO port and updates
  // _portInputState accordingly.
  //  When not in immediate mode, it initiates a request using the request block
  //  and returns. When the request completes, _processCompletion finishes the
  //  operation.
  ...
  void _processCompletion(uint8_t status) override {
    if (status == I2C_STATUS_OK)
      _portInputState =
          (((uint16_t)inputBuffer[1] << 8) | inputBuffer[0]) | _portMode;
    else
