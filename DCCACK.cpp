...
void DCCACK::Setup(int cv, byte byteValueOrBitnum, ackOp const program[],
...
// checkRessets return true if the caller should yield back to loop and try
// later.
...
void DCCACK::setAckBaseline() {
  ...
  if (Diag::ACK)
    DIAG(F("ACK baseline=%d/%dmA Threshold=%d/%dmA Duration between %uus and "
           "%uus"),
         baseline, progDriver->raw2mA(baseline), ackThreshold,
         progDriver->raw2mA(ackThreshold), minAckPulseDuration,
         maxAckPulseDuration);
}
...
byte DCCACK::getAck() {
  ...
  if (Diag::ACK)
    DIAG(F("%S after %dmS max=%d/%dmA pulse=%uuS samples=%d gaps=%d"),
         ackDetected ? F("ACK") : F("NO-ACK"), ackCheckDuration, ackMaxCurrent,
         progDriver->raw2mA(ackMaxCurrent), ackPulseDuration, numAckSamples,
         numAckGaps);
  if (ackDetected)
...
void DCCACK::loop() {
  while (ackManagerProg) {
    ...
    // (typically waiting for a reset counter or ACK waiting, or when all
    // finished.)
    switch (opcode) {
    case BASELINE:
    ...
    case W1: // write 1 bit
    {
      if (checkResets(RESET_MIN))
        return;
      if (Diag::ACK)
        DIAG(F("W%d cv=%d bit=%d"), opcode == W1, ackManagerCv,
             ackManagerBitNum);
      byte instruction =
          WRITE_BIT | (opcode == W1 ? BIT_ON : BIT_OFF) | ackManagerBitNum;
      byte message[] = {DCC::cv1(BIT_MANIPULATE, ackManagerCv),
                        DCC::cv2(ackManagerCv), instruction};
      DCCWaveform::progTrack.schedulePacket(message, sizeof(message),
                                            PROG_REPEATS);
      setAckPending();
    ...
    case WB: // write byte
    {
      ...
      byte message[] = {DCC::cv1(WRITE_BYTE, ackManagerCv),
                        DCC::cv2(ackManagerCv), ackManagerByte};
      DCCWaveform::progTrack.schedulePacket(message, sizeof(message),
                                            PROG_REPEATS);
      setAckPending();
      callbackState = AFTER_WRITE;
    ...
    case VB: // Issue validate Byte packet
    {
      ...
      byte message[] = {DCC::cv1(VERIFY_BYTE, ackManagerCv),
                        DCC::cv2(ackManagerCv), ackManagerByte};
      DCCWaveform::progTrack.schedulePacket(message, sizeof(message),
                                            PROG_REPEATS);
      setAckPending();
    ...
    case V1: // Issue validate bit=0 or bit=1  packet
    {
      if (checkResets(RESET_MIN))
        return;
      if (Diag::ACK)
        DIAG(F("V%d cv=%d bit=%d"), opcode == V1, ackManagerCv,
             ackManagerBitNum);
      byte instruction =
          VERIFY_BIT | (opcode == V0 ? BIT_OFF : BIT_ON) | ackManagerBitNum;
      byte message[] = {DCC::cv1(BIT_MANIPULATE, ackManagerCv),
                        DCC::cv2(ackManagerCv), instruction};
      DCCWaveform::progTrack.schedulePacket(message, sizeof(message),
                                            PROG_REPEATS);
    ...
    case ITCBV: // If True callback(byte) - Verify
      if (ackReceived) {
        if (ackManagerByte == ackManagerByteVerify) {
          ackRetrySum++;
          LCD(1, F("v %d %d Sum=%d"), ackManagerCv, ackManagerByte,
              ackRetrySum);
        }
        callback(ackManagerByte);
        return;
    ...
    case MERGE: // Merge previous Validate zero wack response with byte value
                // and update bit number (use for reading CV bytes)
      ackManagerByte <<= 1;
    ...
    case COMBINELOCOID:
      ...
      callback(LONG_ADDR_MARKER |
               (ackManagerByte + ((ackManagerStash - 192) << 8)));
      return;
...
void DCCACK::checkAck(byte sentResetsSincePacket) {
  ...
  // An ACK is a pulse lasting between minAckPulseDuration and
  // maxAckPulseDuration uSecs (refer @haba)
  ...
  if (ackPulseDuration >= minAckPulseDuration &&
      ackPulseDuration <= maxAckPulseDuration) {
    ackCheckDuration = millis() - ackCheckStart;
    ackDetected = true;
    ackPending = false;
  ...
  ackPulseStart = 0; // We have detected a too-short or too-long pulse so ignore
                     // and wait for next leading edge
}
