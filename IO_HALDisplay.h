...
template <class T> class HALDisplay : public IODevice, public DisplayInterface {
private:
...
public:
  ...
  static void create(uint8_t displayNo, I2CAddress i2cAddress, int width,
                     int height) {
    if (checkNoOverlap(0, 0, i2cAddress))
...
protected:
  ...
  void screenUpdate() {
    ...
    if (!_displayDriver->isBusy()) {
      ...
      if (_charPosToScreen >= _numCols) {
        ...
        if (_rowGeneration[_rowNoToScreen] !=
            _lastRowGeneration[_rowNoToScreen]) {
          ...
          _charPosToScreen =
              0; // Prepare to output first character on next entry
        } else {
          // Row not changed, don't bother writing it.
        }
      } else {
        // output character at current position
        _displayDriver->writeNative(
            _buffer[_rowNoToScreen * _numCols + _charPosToScreen++]);
      }
    }
    return;
  }
  ...
  void _loop(unsigned long) override { screenUpdate(); }

  // Display information about the device.
  void _display() {
    DIAG(F("HALDisplay %d configured on addr %s"), _displayNo,
...
public:
  void _displayLoop() override { screenUpdate(); }
  ...
  void _setRow(byte line) override {
    if (line == 255) {
      ...
      for (int row = 1; row < _numRows; row++) {
        strncpy(&_buffer[(row - 1) * _numCols], &_buffer[row * _numCols],
                _numCols);
