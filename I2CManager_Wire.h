    ...
    uint8_t data = (subBus == SubBus_All)    ? 0xff
                   : (subBus == SubBus_None) ? 0x00
                                             ...
                                             0x04 | subBus; // NB Only 2 or 4
                                                            ...
                                                            // respectively
#else
                                             // Default behaviour for most MUXs
                                             // is to use a mask with a bit set
                                             // for the subBus to be enabled
                       1 << subBus;
#endif
    Wire.write(&data, 1);
 ...
 ***************************************************************************/
uint8_t I2CManagerClass::write(I2CAddress address, const uint8_t buffer[],
                               uint8_t size, I2CRB *rb) {
  uint8_t status, muxStatus;
  uint8_t retryCount = 0;
  // If request fails, retry up to the defined limit, unless the NORETRY flag is
...
#ifdef I2C_EXTENDED_ADDRESS
    // Deselect MUX if there's more than one MUX present, to avoid having
 ...
 *on Wire)
 ***************************************************************************/
uint8_t I2CManagerClass::write_P(I2CAddress address, const uint8_t buffer[],
                                 uint8_t size, I2CRB *rb) {
 ...
 *operation on Wire) If fewer than the number of requested bytes are received,
 *status is I2C_STATUS_TRUNCATED.
 ***************************************************************************/
uint8_t I2CManagerClass::read(I2CAddress address, uint8_t readBuffer[],
                              uint8_t readSize, const uint8_t writeBuffer[],
                              uint8_t writeSize, I2CRB *rb) {
  uint8_t status, muxStatus;
  uint8_t nBytes = 0;
  uint8_t retryCount = 0;
  // If request fails, retry up to the defined limit, unless the NORETRY flag is
  ...
  do {
    ...
    // Deselect MUX if there's more than one MUX present, to avoid having
    // multiple ones selected
    if (_muxCount > 1 && muxStatus == I2C_STATUS_OK && address != 0 &&
        address.muxNumber() != I2CMux_None) {
      muxSelect({address.muxNumber(), SubBus_None});
    }
    if (muxStatus != I2C_STATUS_OK)
      status = muxStatus;
...
void I2CManagerClass::queueRequest(I2CRB *req) {
  ...
  case OPERATION_REQUEST:
    read(req->i2cAddress, req->readBuffer, req->readLen, req->writeBuffer,
         req->writeLen, req);
    break;
  }
}
